...
\vspace{2em}
{\small An Axona Explainer \quad v0.4.23 \quad 2026-05-22\par}
\end{center}
\vfill
\null
\newpage
...
% v0.4.23: tufte-handout routes \caption inside figure/figure* to the
...
Axona keeps them apart with \textbf{two contracts}.\sidenote{The DHT contract is the public surface of the \texttt{AxonaPeer} class in the \texttt{@axona/protocol} kernel; the Transport contract is the abstract base alongside it. Concrete Transports include \texttt{Transport.sim()} for the in-browser simulator, \texttt{Transport.web()} for WebRTC in real browsers, and \texttt{Transport.node()} for headless servers.}

The first contract---call it the \textbf{DHT contract}---is what the application above sees. An app like a chat client doesn't care how routing works internally; it just wants to \emph{do things}. So the DHT exposes an \texttt{AxonaPeer} surface organised into five clusters: \textbf{lifecycle} (\texttt{start}, \texttt{stop}, \texttt{join}, \texttt{leave}); \textbf{pub/sub} (the four verbs from Section~VII---\texttt{pub}, \texttt{sub}, \texttt{pull}, \texttt{metrics}); \textbf{direct messaging} (\texttt{send}, \texttt{notify}, \texttt{onMessage}) for unicast traffic outside the feed; \textbf{mesh introspection} (\texttt{peers}, \texttt{onPeerJoin}, \texttt{onPeerLeave}, \texttt{lookup}) so the application can see what's around it; and \textbf{telemetry} (\texttt{health}, \texttt{onLog}, \texttt{onError})---a way for the application to \emph{watch} what the protocol is doing without being able to mess with it.
