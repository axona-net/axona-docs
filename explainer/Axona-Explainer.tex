...
\vspace{2em}
{\small An Axona Explainer \quad v0.4.04 \quad 2026-05-22\par}
\end{center}
\vfill
\null
\newpage
...
\begin{axis}[
  ...
  \addplot[gray, line width=0.9pt, mark=*, mark size=1.5pt] coordinates {(1,410) (2,842) (3,548)};
  ...
  % G-DHT
  \addplot[gray!70!black, line width=0.9pt, mark=*, mark size=1.5pt] coordinates {(1,268) (2,826) (3,291)};
  \node[font=\scriptsize, color=inkmuted, anchor=west] at (axis cs:3.05,291) {G-DHT};

  % Axona (accent)
  \addplot[rust, line width=1.4pt, mark=*, mark size=2pt] coordinates {(1,240) (2,272) (3,264)};
  \node[font=\scriptsize, color=rust, anchor=west] at (axis cs:3.05,264) {Axona};

\end{axis}
\end{tikzpicture}
\caption[Latency vs.\ network size]{Global lookup latency versus network size, plotted against the Dabek $3\delta$ analytical floor (gray dashed). Kademlia and G-DHT stay multiple $\delta$ above the floor as the network grows. Axona hugs the floor at $1.28$--$1.33\times$. The remaining $\sim$25--30\% overhead matches what the geometric series predicts for 4--5 hops where an ideal protocol takes 3.}
\label{fig:floor}
\end{figure*}

\textbf{Axona hits $1.33\times$ the floor} at 25{,}000 nodes ($272$~ms vs a $204$~ms $3\delta$ floor at $\delta = 68.0$~ms). Plain Kademlia sits at $4.1\times$ the floor on the same network ($842$~ms). The $\sim$30\% overhead Axona still pays has a clean structural explanation: it takes about 5 hops where an ideal protocol would take 3, and each ``extra'' hop costs about $\delta/2$, exactly as the geometric series predicts.

Kademlia stays multiple $\delta$ above the floor as the network grows. Axona approaches the floor by learning per-hop locality---its Action-Potential scoring weights short-RTT edges so heavily that even a hop ``inefficient'' by raw count is short in wall-clock time. Kademlia's distance-only metric ignores RTT, so each of its hops costs the average inter-node delay rather than the geometric-halved one.
