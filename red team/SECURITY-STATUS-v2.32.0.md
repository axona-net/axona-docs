...
| ID | Sev | Issue | Plan | Effort |
|---|---|---|---|---|
| **E-1** | Med sev / **High strategic** | Targeted address-grinding: an adversary grinds an identity into a chosen topic's K-closest set → eclipse / censorship-by-omission / interest-surveillance | **DECIDED (2026-06-09): memory-hard PoW** on identity minting; **Vivaldi/RTT rejected** (the 8-bit prefix is a routing area-code, not a residency requirement — verifying RTT against it breaks "works anywhere" + location privacy, and can't touch the 256-bit hash where grinding happens). PoW is pubkey-bound, per-role difficulty, **shipped at difficulty 0 now** (no UX cost; avoids a future identity-format flag-day), dialed up under threat. Also answers the publishID anti-flood anchor. Full record: [`architecture/E-1-Placement-Defense-v0.1.md`](../architecture/E-1-Placement-Defense-v0.1.md) | L |
