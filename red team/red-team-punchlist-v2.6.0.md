...
| 19 | **E-1** | Med | Targeted-placement defense: proof-of-work on identity generation **or** Vivaldi RTT-coordinate cross-check. | **L** | **Decision required** — both options are decentralized (constraint-OK), but PoW taxes honest join UX and Vivaldi adds significant complexity. Recommend deciding the *approach* before scheduling. See [Axona vs. Vivaldi](../architecture/Axona-vs-Vivaldi-v0.1.md) for why Vivaldi is *not* equivalent to Axona's existing measured-RTT metric (predicted-coordinate model error + manipulation surface vs. measured RTT + self-asserted prefix). |
