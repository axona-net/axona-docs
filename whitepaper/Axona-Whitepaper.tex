...
\vspace{2em}
{\small Version 0.3.54 \quad Simulator v0.91.0 \quad 2026-05-21\par}
\vfill
\end{fullwidth}
\newpage
...
\textbf{Result (May 2026, v1.1.2 / sim v0.91.0).} The
hypothesis is falsified.  With \emph{zero} geographic
information at $25{,}000$ nodes, all three adaptive
protocols beat Kademlia by $\sim 40\%$ — Axona by $43\%$,
NX-17 and NH-1 each by $39\%$.  See
Table~\ref{tab:geobits-ablation} in the parallel ablation
section below for the full numbers.  Lookup success
remains $100\%$ across all five protocols at \texttt{geoBits}
$= 0$ on every distance band.  The learning mechanisms do
real work, attributable cleanly to the synaptome shaping
itself by traffic.

NH-1 and NX-17 are now indistinguishable at \texttt{geoBits}
$= 0$ ($524$ vs $523$ ms global), closing the gap NH-1
carried in earlier sim versions.  The $\sim 30\%$
parameter-count reduction NH-1 trades for over NX-17 no
longer comes with a measurable peak-performance cost in the
no-locality regime.  Axona's $\sim 8\%$ further drop below
NH-1/NX-17 is the production-Axona dividend from the
TransportAxonaEngine churn-cleanup work (v0.85.0–v0.89.0)
and the v1.1.2 per-hop live-RTT lookup-latency change.
...
\midrule
\KDHT       & $845$ & $852$ & --- \\
G-DHT       & $820$ & $1060$ & $24\%$ slower \\
NX-17       & $270$ & $523$ & $39\%$ faster \\
NH-1        & $261$ & $524$ & $39\%$ faster \\
\textbf{Axona} & $\mathbf{254}$ & $\mathbf{482}$ & $\mathbf{43\%}$ \textbf{faster} \\
\bottomrule
\end{tabular}
\end{table}

The ablation falsifies the hypothesis.  With \emph{zero}
geographic information, all three adaptive protocols still
beat Kademlia by $\sim 40\%$; Axona alone leads the field
at $43\%$.  The learning is doing real work, attributable
cleanly to the synaptome shaping itself by traffic.

G-DHT is the only protocol that gets \emph{worse} at
\texttt{geoBits} $= 0$ ($820 \to 1060$ ms): its entire
...
Axona's $43\%$ lead over Kademlia at \texttt{geoBits} $= 0$
is the production-Axona dividend on top of the research
NH-1/NX-17 kernel: the TransportAxonaEngine churn-cleanup
work (dht-sim v0.85.0--v0.89.0) and the v1.1.2 per-hop
live-RTT lookup-latency change cumulatively dropped Axona
below the NH-1/NX-17 baseline.  NH-1 and NX-17 are now
indistinguishable at \texttt{geoBits} $= 0$ ($524$ vs $523$
ms) — a year of kernel parity work having closed the gap
NH-1 carried in the earlier 12-rule consolidation.  The
ablation no longer shows a measurable consolidation cost:
NH-1 retains its $\sim 30\%$ parameter-count reduction
vs NX-17 without sacrificing peak performance at
\texttt{geoBits} $= 0$.
