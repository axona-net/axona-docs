...
\vspace{2em}
{\small Version 0.3.57 \quad Simulator v0.94.0 \quad 2026-05-23\par}
\vfill
\end{fullwidth}
\newpage
...
\item[\textsc{node ID}]
A 264-bit integer addressing a node in the network. In \GDHT and
\NDHT the high \textsc{geoBits} = 8 bits encode the S2 cell
prefix; the low 256 bits are the SHA-256 hash of the node's
public key (Ed25519). See \S\ref{ch:locality}.

\item[\textsc{publisher-prefix topic ID}]
The construction $\mathrm{topicId} = (\text{publisher's S2 prefix})
\Vert H_{256}(\mathrm{topicName})$. The high bits pin the topic's
...
remaining 256 bits of node ID give cryptographic-strength
intra-cell collision resistance.\marginnote{Earlier experiments tried 4 and 16. Four
...
\newthought{The node ID} is a 264-bit integer assembled from two
fields:
...
\;\Vert\;
\underbrace{H_{256}(\mathit{publicKey})}_{\text{256 bits}}
\]
...
low 256 bits are the SHA-256 hash of the node's Ed25519 public
key, providing intra-cell uniqueness while keeping the ID bound
to a stable cryptographic identity.  On the wire the 264-bit
identifier is encoded as 66 lowercase hex characters.
...
256-bit hash dominates and XOR distance is essentially random)
...
\newthought{The address} of a topic is a 264-bit integer. \NDHT
constructs it as:
...
\;\Vert\;
\underbrace{H_{256}(\mathit{topicName})}_{\text{256 bits}}
\]

The high 8 bits come from the publisher's S2 cell prefix; the low
256 bits are the SHA-256 hash of the topic name. The address is deterministic:
...
\textbf{Result (May 2026, v1.1.2 / sim v0.93.0).} The
hypothesis is falsified.  With \emph{zero} geographic
information at $25{,}000$ nodes, all three neuromorphic
protocols beat Kademlia by $\sim 39\%$ on global lookups
(NX-17 $523$ ms, NH-1 $524$ ms, Axona $513$ ms, vs Kademlia
$852$ ms).  See Table~\ref{tab:geobits-ablation} in the
parallel ablation section below for the full numbers.
Lookup success remains $100\%$ across all five protocols at
\texttt{geoBits} $= 0$ on every distance band.  The learning
mechanisms do real work, attributable cleanly to the
synaptome shaping itself by traffic.

NX-17, NH-1, and Axona converge within $\sim 2\%$ on
non-churn routing at \texttt{geoBits} $= 0$, exactly as the
shared-\texttt{AxonaPeer}-kernel architecture predicts.
The deployed-Axona dividend appears in the churn cell:
$382$ ms vs $585$ (NX-17) / $614$ (NH-1) at $5\%$ churn,
\texttt{geoBits} $= 0$ --- a $35$--$38\%$ saving from the
explicit dying-nodeId sweep that
\texttt{TransportAxonaEngine.removeNode} performs on every
surviving peer's synaptome (work landed v0.85.0--v0.89.0).
This cleanup advantage is geometric, not prefix-dependent ---
it shows at \texttt{geoBits} $= 8$ ($239$ ms vs $\sim 292$ ms)
and \texttt{geoBits} $= 0$ ($382$ ms vs $\sim 600$ ms) in
the same architectural proportion.
...
(ablation). All five protocols, \texttt{@axona/protocol}
v1.1.2 / sim v0.91.0, May 2026.  $100\%$ success on every
cell.  The drop with no prefix is what the geographic
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Protocol} & \textbf{gB=8 (ms)} & \textbf{gB=0 (ms)} & \textbf{Δ vs Kad (gB=0)} \\
\midrule
\KDHT       & $842$ & $852$ & --- \\
G-DHT       & $826$ & $1060$ & $24\%$ slower \\
NX-17       & $265$ & $523$ & $39\%$ faster \\
NH-1        & $260$ & $524$ & $39\%$ faster \\
\textbf{Axona} & $272$ & $\mathbf{513}$ & $\mathbf{40\%}$ \textbf{faster} \\
\bottomrule
\end{tabular}
\end{table}

The ablation falsifies the hypothesis.  With \emph{zero}
geographic information, all three neuromorphic protocols
(NX-17, NH-1, Axona) beat Kademlia by $\sim 39\%$ on global
lookups, within RNG noise of each other.  The learning is
doing real work, attributable cleanly to the synaptome shaping
itself by traffic.

G-DHT is the only protocol that gets \emph{worse} at
\texttt{geoBits} $= 0$ ($826 \to 1060$ ms): its entire
strategy is ``follow the prefix.''  With no prefix to
follow, it falls back to a slower no-progress-limited search
and loses the locality benefit that defines it.

NX-17, NH-1, and Axona converge within $\sim 2\%$ on
non-churn routing at \texttt{geoBits} $= 0$ (523 / 524 / 513
ms global), as the shared-\texttt{AxonaPeer}-kernel
architecture predicts.  The \emph{deployed} Axona dividend
appears in the churn cell: at $5\%$ churn, \texttt{geoBits}
$= 0$, Axona lookups complete at $382$ ms vs NX-17's $585$
and NH-1's $614$ --- a $35$--$38\%$ saving from the explicit
dying-nodeId sweep that \texttt{TransportAxonaEngine.removeNode}
performs on every surviving peer's synaptome (work landed in
sim v0.85.0--v0.89.0).  The cleanup advantage is geometric,
not prefix-dependent.
...
\paragraph{Pub/sub application API --- the four verbs.}
Above the axonal-tree primitive sits a feed-style API on the
kernel \texttt{AxonaPeer} (in \texttt{@axona/protocol} v1.0+;
the routing-and-fan-out lives in \texttt{AxonManager}):

\begin{itemize}\itemsep2pt
\item \texttt{peer.pub(topic, message, opts)} --- author a new
  signed envelope into one of your topics.
\item \texttt{peer.sub(topic, handler, opts)} --- attach to a
  topic and receive future posts via the axonal tree, plus
  any replay-cached messages newer than the subscriber's
  \texttt{lastSeenTs}.
\item \texttt{peer.pull(msgId, opts)} --- fetch any post by
  message id from its topic's relay tree (single-count
  \texttt{pull\_count} invariant on the first FOUND response).
\item \texttt{peer.metrics(topic, opts)} --- aggregate
  \texttt{publishes}, \texttt{subscribers}, and
  \texttt{reshare\_count} for a topic the caller owns.
\end{itemize}

Resharing is not a separate verb: an application reshares by
re-issuing \texttt{peer.pub} on the upstream signed envelope.
The first role-bearing relay on the notification path sees
the duplicate \texttt{msgId} and bumps \texttt{reshare\_count},
which \texttt{metrics} then surfaces back to the publisher.

Encryption, schema, and ordering belong to the application
above this layer; the protocol carries opaque message bytes.
The kernel's pub/sub cascade smoke test verifies the counter
invariants end-to-end (see
\texttt{test/smoke\_kernel\_regression.mjs} and the in-sim
\texttt{smoke:three} mesh-fail recovery suite).
...
\item[\textsc{node ID}] 264-bit identifier. High 8 bits = S2
cell prefix; low 256 bits = $H_{256}$(public key) (SHA-256 of the
Ed25519 public key).
Chapter~\ref{ch:locality}.
