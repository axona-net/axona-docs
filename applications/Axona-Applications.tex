...
\vspace{2em}
{\small An Axona Applications Note \quad v0.3 \quad 2026-06-01\par}
\end{center}
\vfill
\null
\newpage
...
\newthought{The applications above} are open-source systems that
\emph{use} a DHT. There is a second class of target that is not an
application at all but a \emph{business model}: companies whose product
\emph{is} pub/sub. They exist because operating WebSocket fan-out at
scale --- connection management, presence, history, global points of
presence --- is hard, so they run it for you and meter it. Axona changes
the economics directly: fan-out becomes a property of the mesh, the
per-message meter disappears, and so does the operator who could read
the traffic. These services sort into four tiers by how cleanly Axona
displaces them.
...
\bottomrule
\end{tabular}
\caption{Tier-1 realtime pub/sub features and their Axona equivalents.}
\label{tab:paas-mapping}
\end{table*}
