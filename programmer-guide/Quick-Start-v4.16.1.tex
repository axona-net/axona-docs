...
current \texttt{@axona/protocol} \textbf{v4.16.1} API (kernel 4.16.1).
...
uses testnet + the \texttt{\#v4.16.1} pin. To target production instead,
...
\tightlist
\item
  \href{Axona-Programmer-Guide-v4.16.1.md}{Programmer Guide} --- the
  five ideas + recipes for real apps.
\item
  \href{Axona-API-Reference-v4.16.1.md}{API Reference} --- every
  exported symbol.
\item
  \href{Axona-AI-Grounding-v4.16.1.md}{AI Grounding} --- building with
...
\ExtensionTok{npm}\NormalTok{ install github:axona{-}net/axona{-}protocol\#v4.16.1}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ \{ deriveTopicId}\OperatorTok{,}\NormalTok{ KERNEL\_VERSION \} }\ImportTok{from} \StringTok{\textquotesingle{}@axona/protocol\textquotesingle{}}\OperatorTok{;}
...
\KeywordTok{const}\NormalTok{ BRIDGE }\OperatorTok{=} \StringTok{\textquotesingle{}wss://testnet.axona.net\textquotesingle{}}\OperatorTok{;}           \CommentTok{// live testnet bridge (kernel 4.16.1)}
...
\href{Axona-Programmer-Guide-v4.16.1.md}{Programmer Guide} \\
Look up a specific symbol & \href{Axona-API-Reference-v4.16.1.md}{API
Reference} \\
\end{longtable}
}
...
mismatch. The testnet bridge runs kernel 4.16.1 (wire 4.0); install
\texttt{github:axona-net/axona-protocol\#v4.16.1}. Note this also fires
