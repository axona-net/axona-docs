...
*(kernel 4.48.0 · testnet — versioned with the protocol it describes; revised
...
- **Protocol kernel**: [@axona/protocol](https://github.com/axona-net/axona-protocol) (v4.48.0)
- **Wire version**: 4.0 (`WIRE_VERSION`); kernel version 4.48.0 (`KERNEL_VERSION`)
- **Live networks**: both run the 4.x line. **Testnet** (`wss://testnet.axona.net`)
  tracks the newest kernel — 4.48.0, the version this guide describes. The
...
curl https://bridge.example.net/healthz     # → {"status":"ok","version":…,"kernelVersion":"4.48.0"}
```
...
| `BRIDGE_DIRECTORY_MIN_UPTIME_MS` | How long a bridge must have been up before it advertises. Default `300000` (5 min). See "The establishment gate" below |
| `BRIDGE_DIRECTORY_MIN_PEERS` | How many mesh peers it must have before it advertises. Default `3` |
| `BRIDGE_NEVER_ROOT` | `1`/unset = a bridge never holds a topic role (the default, and the invariant). `0` restores pre-4.46 behaviour — only for a bridge-only network with no relays at all |
...
A bridge does **not** advertise itself the moment it starts. It waits until it is
*established* — `BRIDGE_DIRECTORY_MIN_UPTIME_MS` of uptime **and**
`BRIDGE_DIRECTORY_MIN_PEERS` mesh peers — and only then publishes its directory
entry. The hourly heartbeat keeps the entry fresh from there.

Two reasons, and the second is the one that bites:

1. **It is the honest behaviour.** A bridge that announces "connect to me" before
   it can carry traffic is advertising a promise it cannot keep. A launch publish
   is worth nothing anyway: nobody is listening to a mesh the bridge has not
   joined yet.
2. **At launch a bridge is TERMINAL for its own directory topic.** Its synaptome
   is empty, so nobody is closer to that address than it is. Combined with the
   bridge fence (`BRIDGE_NEVER_ROOT`, below) it would refuse to root the very
   topic it is publishing — and before kernel 4.48.0 the declined message was
   re-routed to the only node available, itself, in an unbounded synchronous
   loop. That took a production bridge down for ~50 minutes on 2026-07-27.

Both halves are now fixed independently: the gate removes the trigger (bridge
2.100.0) and the kernel terminates an undeliverable declined message rather than
retrying it (4.48.0). Either alone is sufficient; both is correct.

**What you will see in the log** while it waits:

```
directory:awaiting-establishment {"upMs":15703,"peers":44,"needUptimeMs":300000,"needPeers":3}
directory:established           {"upMs":302076,"peers":45}
directory:published             {"url":"wss://bridge.axona.net","reason":"established","region":"eagle"}
```

A bridge sitting in `awaiting-establishment` is healthy and serving traffic — it
is simply not yet listed for *other* people to bootstrap through.

### A bridge is a bridge — it never holds a topic role

A bridge transports and introduces. It is never a root, backup, child or holder
for any topic, at any load, in any topology. This is enforced in three
independent places, so do not assume removing one is enough:

- **Routing** skips a bridge as a next hop entirely (oldest guard).
- **`host()` was removed** from the directory publisher (2026-07-25).
- **`neverRoot: true`** makes the kernel refuse every role at the HARD tier —
  the admission floor may *not* override it, even when every other candidate is
  saturated or still in grace.

`BRIDGE_NEVER_ROOT=0` exists only for a bridge-only network during a cold start
with no relays at all. If you find yourself reaching for it in production, the
answer is more relays, not a bridge holding data.
...
`axona-relay v0.67.0 [EPHEMERAL] (kernel v4.48.0)` — and the bridge reports the
...
- **[Programmer Guide](Axona-Programmer-Guide-v4.48.0.md)** — build the peer that
  talks to these services.
- **[API Reference](Axona-API-Reference-v4.48.0.md)** — `host()` / `unhost()`,
