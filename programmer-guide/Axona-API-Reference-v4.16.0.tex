...
\textbf{any} topic, open or owned. As of kernel \textbf{v4.3.0} metrics
are no longer scatter- gathered: a topic's root publishes a signed
snapshot to \texttt{metricTopic(T)} every \textasciitilde20 s (the relay
fleet runs this), and \texttt{metrics()} subscribes to that \emph{open}
metric topic briefly and returns the freshest snapshot via
replay-on-subscribe. Owned topics' metrics are \textbf{public too} ---
anyone who can derive the id can read them --- so there is no owner
gate.
...
The producer side of the convention. A topic's primary root publishes a
signed metric snapshot to a \emph{derived, open} topic every
\textasciitilde20 s --- for \textbf{both open and owned} data topics
(v4.3.0). Compute that topic with \texttt{metricTopic()} and
\texttt{sub()} it: you get the latest snapshot via replay-on-subscribe
plus every update --- one subscription instead of a poll --- and,
because snapshots are ordinary messages that age out at the 48 h hold
ceiling, a \textbf{rolling \textasciitilde48 h history for free} to plot
trends. This is also how you subscribe to an \textbf{owned} topic's
metrics without owning it.
