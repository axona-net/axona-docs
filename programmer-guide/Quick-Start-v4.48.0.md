...
`@axona/protocol` **v4.48.0** API (kernel 4.48.0). One Node process connects
...
> (`wss://testnet.axona.net`) tracks the newest release — 4.48.0, which this
...
const BRIDGE = 'wss://testnet.axona.net';           // live testnet bridge (kernel 4.48.0)
...
bridge runs kernel 4.48.0 (wire 4.0); install
`github:axona-net/axona-protocol#v4.48.0`. Production runs the same wire-4 line
