...
Reference for every public symbol exported from `@axona/protocol` v4.48.0.
...
below is verified against the v4.48.0 kernel source.

> **Which network?** Both live networks run the 4.x line (wire 4.0).
> **Testnet** (`wss://testnet.axona.net`) tracks the newest kernel — v4.48.0,
...
> `github:axona-net/axona-protocol#v4.48.0`.
...
> [AI Grounding](Axona-AI-Grounding-v4.48.0.md) file — the application
...
### `connect(options)` → `Promise<{ peer, author, nodeIdentity, transport, status, disconnect }>` *(kernel 4.16.0; barrel export 4.48.0)*
...
//   admission: {                       // kernel 4.46.0+
//     seated, saturated, refusals,
//     capacity: {                      // kernel 4.47.0+ — MEASURED, not counted
//       roles, overdue, overdueFrac, unserviced, worstAgeMs,
//       servicePressure,               // worstAgeMs / DROP_MS
//       tickLagMs, tickLagMaxMs, tickDurMs, tickStalls,
//       helloPressure,                 // tickLag / HELLO_DEADLINE_MS
//     },
//   },
// }
```

`admission` reports whether this node will accept new roles, and why not.
`seated` is false during the post-join grace period; `saturated` is true when the
node is at capacity.

**`capacity` is measured, never counted.** `roles` is inventory, not a limit —
production nodes routinely run several hundred. The two numbers that mean
something are the *pressures*, each a ratio against a real protocol deadline:

| Field | Meaning | Saturated at |
|---|---|---|
| `servicePressure` | age of the least-recently-serviced role ÷ `DROP_MS` | ≥ 0.6 |
| `helloPressure` | observed tick lag ÷ `HELLO_DEADLINE_MS` | ≥ 0.6 |

A node holding 400 roles that services all of them on time is **healthy**. A node
holding 40 that cannot is **saturated**. Poll the pressures, not `roles.length` —
an earlier release used a role *count* as the predicate and it was meaningless,
because the ceiling was exceeded twelvefold in ordinary operation.

**Refusal is normal, and refusal is not loss.** A node that declines a role logs
it; a declined message is forwarded to a node that can take it. If there is no
such node the message stops and is logged `undeliverable` rather than being
retried forever — see the Services Guide on the bridge fence for the one case
where refusal is absolute.
...
const transport = webTransport({
  ...
  peerVersion: '4.48.0',                   // your app version (gated by the bridge)
  reconnect:   true,
});
...
KERNEL_VERSION       // '4.48.0'   — kernel semver (npm release tag)
...
### What changed in v4.39.0–v4.48.0 (2026-07-24; v4.48.0 on testnet)
...
- **`connect()` (v4.16.0; barrel export + single-bootstrap consolidation v4.48.0)** — the one-call bootstrap above; no other API change.
