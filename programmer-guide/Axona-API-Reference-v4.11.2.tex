...
interoperate. Install \texttt{github:axona-net/axona-protocol\#v4.11.2}
...
\tightlist
\item
  \href{Quick-Start-v4.11.2.md}{Quick Start} --- 5-minute working
  roundtrip.
\item
...
\subsection{What changed in the 4.x line (since
...
\item
  \textbf{Nearest-replica reads (v4.11.1--v4.11.2).} \texttt{pull} is
...
\subsection{What changed since v2.x}\label{what-changed-since-v2.x}
...
\KeywordTok{const}\NormalTok{ transport }\OperatorTok{=} \FunctionTok{webTransport}\NormalTok{(\{}
  ...
  \DataTypeTok{peerVersion}\OperatorTok{:} \StringTok{\textquotesingle{}4.11.2\textquotesingle{}}\OperatorTok{,}                   \CommentTok{// your app version (gated by the bridge)}
...
\NormalTok{KERNEL\_VERSION       }\CommentTok{// \textquotesingle{}4.11.2\textquotesingle{}   — kernel semver (npm release tag)}
...
\texttt{@axona/protocol} v3.x is stable for the application surface
