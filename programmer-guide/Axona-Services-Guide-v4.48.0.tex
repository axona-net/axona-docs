...
\vspace{2em}
{\small The Axona Services Guide \quad v4.48.0 \quad 2026-07-27\par}
\end{center}
\vfill
\null
\newpage
...
\emph{(kernel 4.48.0 · testnet --- versioned with the protocol it
describes; revised 2026-07-23 with the one-command relay-fleet launcher
and the relay update procedure, §3)}
...
\item
  ...
  \tightlist
  \item
    \href{Quick-Start-v4.48.0.md}{Quick Start} ·
    \href{Axona-Programmer-Guide-v4.48.0.md}{Programmer Guide} ·
    \href{Axona-API-Reference-v4.48.0.md}{API Reference} --- the
    library.
  \item
    \href{Axona-AI-Grounding-v4.30.0.md}{AI Grounding} --- building with
    an AI assistant.
  \item
...
\ExtensionTok{curl}\NormalTok{ https://bridge.example.net/healthz     }\CommentTok{\# → \{"status":"ok","version":…,"kernelVersion":"4.48.0"\}}
\end{Highlighting}
\end{Shaded}
...
\texttt{off}) \\
\texttt{BRIDGE\_DIRECTORY\_MIN\_UPTIME\_MS} & How long a bridge must
have been up before it advertises. Default \texttt{300000} (5 min). See
``The establishment gate'' below \\
\texttt{BRIDGE\_DIRECTORY\_MIN\_PEERS} & How many mesh peers it must
have before it advertises. Default \texttt{3} \\
\texttt{BRIDGE\_NEVER\_ROOT} & \texttt{1}/unset = a bridge never holds a
topic role (the default, and the invariant). \texttt{0} restores
pre-4.46 behaviour --- only for a bridge-only network with no relays at
all \\
...
\subsubsection{The establishment gate}\label{the-establishment-gate}

A bridge does \textbf{not} advertise itself the moment it starts. It
waits until it is \emph{established} ---
\texttt{BRIDGE\_DIRECTORY\_MIN\_UPTIME\_MS} of uptime \textbf{and}
\texttt{BRIDGE\_DIRECTORY\_MIN\_PEERS} mesh peers --- and only then
publishes its directory entry. The hourly heartbeat keeps the entry
fresh from there.
...
\tightlist
\item
  \textbf{It is the honest behaviour.} A bridge that announces ``connect
  to me'' before it can carry traffic is advertising a promise it cannot
  keep. A launch publish is worth nothing anyway: nobody is listening to
  a mesh the bridge has not joined yet.
\item
  \textbf{At launch a bridge is TERMINAL for its own directory topic.}
  Its synaptome is empty, so nobody is closer to that address than it
  is. Combined with the bridge fence (\texttt{BRIDGE\_NEVER\_ROOT},
  below) it would refuse to root the very topic it is publishing --- and
  before kernel 4.48.0 the declined message was re-routed to the only
  node available, itself, in an unbounded synchronous loop. That took a
  production bridge down for \textasciitilde50 minutes on 2026-07-27.
\end{enumerate}

Both halves are now fixed independently: the gate removes the trigger
(bridge 2.100.0) and the kernel terminates an undeliverable declined
message rather than retrying it (4.48.0). Either alone is sufficient;
both is correct.

\textbf{What you will see in the log} while it waits:
...
A bridge sitting in \texttt{awaiting-establishment} is healthy and
serving traffic --- it is simply not yet listed for \emph{other} people
to bootstrap through.

\subsubsection{A bridge is a bridge --- it never holds a topic
role}\label{a-bridge-is-a-bridge-it-never-holds-a-topic-role}

A bridge transports and introduces. It is never a root, backup, child or
holder for any topic, at any load, in any topology. This is enforced in
three independent places, so do not assume removing one is enough:

\begin{itemize}
\tightlist
\item
  \textbf{Routing} skips a bridge as a next hop entirely (oldest guard).
\item
  \textbf{\texttt{host()} was removed} from the directory publisher
  (2026-07-25).
\item
  \textbf{\texttt{neverRoot:\ true}} makes the kernel refuse every role
  at the HARD tier --- the admission floor may \emph{not} override it,
  even when every other candidate is saturated or still in grace.
\end{itemize}

\texttt{BRIDGE\_NEVER\_ROOT=0} exists only for a bridge-only network
during a cold start with no relays at all. If you find yourself reaching
for it in production, the answer is more relays, not a bridge holding
data.
...
\subsubsection{Running a fleet (one
command)}\label{running-a-fleet-one-command}

To run several relays at once --- on a fresh machine with nothing
installed but Node 20+ and Git --- use the cross-platform launcher
instead of starting each by hand:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{git}\NormalTok{ clone }\AttributeTok{{-}b}\NormalTok{ testnet https://github.com/axona{-}net/axona{-}relay.git}
\BuiltInTok{cd}\NormalTok{ axona{-}relay}
\ExtensionTok{npm}\NormalTok{ install}
\ExtensionTok{node}\NormalTok{ scripts/fleet.mjs }\AttributeTok{{-}{-}region}\NormalTok{ grizzly }\AttributeTok{{-}{-}count}\NormalTok{ 10 }\AttributeTok{{-}{-}network}\NormalTok{ testnet}
\end{Highlighting}
\end{Shaded}

The last line is the whole thing. It takes flags, not environment
variables:
...
\bottomrule\noalign{}
\endlastfoot
\texttt{-\/-region\ \textless{}name\ or\ code\textgreater{}} & Region
every relay in the fleet hosts (e.g.~\texttt{grizzly} or \texttt{0x80}).
Default \texttt{useast}. \\
\texttt{-\/-count\ \textless{}n\textgreater{}} & How many relay
processes to launch. Default \texttt{1}. \\
\texttt{-\/-network\ \textless{}prod\ or\ testnet\textgreater{}} & Which
bridge to bootstrap from. Default \texttt{prod}. \\
\texttt{-\/-bridge\ \textless{}wss-url\textgreater{}} & Explicit bridge
URL (overrides \texttt{-\/-network}). \\
\end{longtable}
}

Each relay is its own child process with a fresh \textbf{ephemeral}
transport identity (re-minted every start), so N relays never collide.
\texttt{Ctrl-C} stops the whole fleet gracefully. It is pure Node ---
identical on Windows, macOS, and Linux (no bash, no global environment).
On Windows this is the supported way to run a fleet, since the
\texttt{start-fleet.sh} operator script is bash-only.

By default the launcher shows a \textbf{live dashboard} --- one fixed
table, one row per relay (node, state, peers, mesh, roles, subs),
redrawn in place --- so you can watch fleet health at a glance instead
of a wall of interleaved scrolling logs. Pass \texttt{-\/-raw} to stream
each relay's raw log lines instead (for debugging a single relay), and
\texttt{-\/-stagger\ \textless{}ms\textgreater{}} to change the gap
between launches (default 600 ms; the spread keeps N simultaneous
handshakes from overwhelming a small bridge host).
...
\subsubsection{Updating a relay}\label{updating-a-relay}

The kernel is \textbf{vendored} into the relay checkout (committed under
\texttt{vendor/axona-protocol/}), so following a new testnet release is
just a pull and a restart --- no separate kernel install:

\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{cd}\NormalTok{ axona{-}relay}
\FunctionTok{git}\NormalTok{ pull                 }\CommentTok{\# brings the new relay + the vendored kernel it ships}
\CommentTok{\# then restart your relay(s):}
\CommentTok{\#   {-} fleet launcher:  Ctrl{-}C, then re{-}run \textasciigrave{}node scripts/fleet.mjs …\textasciigrave{}}
\CommentTok{\#   {-} systemd service: systemctl restart axona{-}relay}
\end{Highlighting}
\end{Shaded}

Each relay prints its versions in the startup banner ---
\texttt{axona-relay\ v0.67.0\ {[}EPHEMERAL{]}\ (kernel\ v4.48.0)} ---
and the bridge reports the kernel it expects at \texttt{/healthz}. Keep
them in step: a relay on an older kernel still connects and relays, but
only relays on the \textbf{current} kernel take part in the newest
behaviours (for example, cooperative bridge graduation, which a relay
must be on kernel ≥ 4.35.0 to honour --- an older relay counts against
the bridge's connection cap but is never released to make room for a
newcomer).

\emph{(Operators who vendor from a local kernel checkout re-vendor with
\texttt{npm\ run\ sync:protocol} instead of \texttt{git\ pull}; end
users who cloned the repo just pull.)}
...
\paragraph{Launching it --- a procedure for an AI
agent}\label{launching-it-a-procedure-for-an-ai-agent}

\emph{Follow these steps in order. Each has a check; do not proceed
until the check passes. Most ``the MCP server won't start'' reports are
step 3 (not restarting) or step 5 (aborting the slow first call) ---
read those twice.}

\textbf{1. Prerequisites (once per machine).} - Node \textbf{≥ 20}:
\texttt{node\ -v}. (WebCrypto and the native transport need it.) - The
\texttt{axona-relay} repository present locally, and its dependencies
installed: \texttt{cd\ axona-relay\ \&\&\ npm\ install}. This step
compiles/fetches the \textbf{\texttt{node-datachannel}} native WebRTC
binary --- \emph{without it the peer cannot open a connection and every
tool call fails}. The kernel is already vendored in the repo (no
separate build). - Sanity-check the server file loads:
\texttt{node\ -\/-check\ src/mcp.js} (exit 0).

\textbf{2. Register the server.} The server is one command:
\texttt{node\ \textless{}abs\textgreater{}/axona-relay/src/mcp.js}.
Register it with the agent runtime. For Claude Code:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{claude}\NormalTok{ mcp add axona }\AttributeTok{{-}{-}}\NormalTok{ node /abs/path/to/axona{-}relay/src/mcp.js}
\CommentTok{\# testnet instead of production:}
\ExtensionTok{claude}\NormalTok{ mcp add axona }\AttributeTok{{-}{-}env}\NormalTok{ RELAY\_NETWORK=testnet }\AttributeTok{{-}{-}}\NormalTok{ node /abs/path/to/axona{-}relay/src/mcp.js}
\end{Highlighting}
\end{Shaded}

or a project-scoped \texttt{.mcp.json} (the two blocks shown above).
\textbf{Use an absolute path}, and make sure \texttt{node} is on the
runtime's \texttt{PATH}.

\textbf{3. Restart / reconnect the agent.} MCP servers are loaded
\textbf{at client startup}. After \texttt{claude\ mcp\ add} (or editing
\texttt{.mcp.json}) you \textbf{must restart or reconnect} the session,
and approve the new project-scoped server once when prompted.
\emph{Skipping this is the number-one reason the tools ``don't
appear''.}

\textbf{4. Confirm the tools are present.} After restart, the tools
appear as \texttt{mcp\_\_axona\_\_axona\_status},
\texttt{mcp\_\_axona\_\_axona\_publish}, etc. If they are absent, the
server didn't load --- return to step 2/3 (path wrong, \texttt{node} not
found, or no restart).

\textbf{5. Bring the peer online --- call \texttt{axona\_status} and
wait for it.} The persistent peer is created \textbf{lazily on the first
tool call} and then waits for the mesh to form. \textbf{The first call
can take several seconds, and up to \textasciitilde30 s on a cold
network --- this is normal; let it return, do not treat the delay as a
failure and do not spam retries} (there is one server process; retrying
can't speed it up and only queues more work). \textbf{Definition of
done:} \texttt{axona\_status} returns an object with a \texttt{nodeId},
an \texttt{authorId}, and mesh health showing \textbf{at least one peer}
(\texttt{mesh.peers\ ≥\ 1} / synaptome ≥ 1). That object \emph{is} the
proof the server launched and joined the network. If it returns with
\textbf{zero} peers, the mesh is still forming --- wait a few seconds
and call \texttt{axona\_status} again.

\textbf{6. Prove a round-trip (optional but recommended).}
\texttt{axona\_publish} a message to a scratch topic, then
\texttt{axona\_pull} the same topic + region --- you should read your
own message back. Publisher and reader \textbf{must use the same
\texttt{region}} (default \texttt{useast}) or they derive different
topic IDs and never meet.

\textbf{Failure modes and fixes:}
...
\bottomrule\noalign{}
\endlastfoot
Tools never appear & server not loaded & restart the agent after
\texttt{claude\ mcp\ add}; verify absolute path + \texttt{node} on
PATH \\
First call hangs \textasciitilde30 s then errors & mesh can't form
(bridge unreachable) & check connectivity;
\texttt{curl\ https://bridge.axona.net/healthz} (or
\texttt{testnet.axona.net}) should return \texttt{\{"status":"ok"\}};
then retry \\
\texttt{TransportError:\ bridge\ socket\ closed\ before\ open} &
transient bridge overload / admission race & \textbf{not fatal} --- wait
a few seconds and call again \\
Cannot find module \texttt{node-datachannel} (or similar native error) &
\texttt{npm\ install} not run & run \texttt{npm\ install} in
\texttt{axona-relay} \\
\texttt{axona\_status} shows a \texttt{nodeId} a \emph{second} running
agent also reports & two MCP processes share the durable identity file
(\texttt{\textasciitilde{}/.axona/claude-mcp-identity.json}) → two peers
with one nodeId collide on the network & give each agent its own
identity: set \texttt{MCP\_AUTHOR\_PATH} to a distinct path per
agent/project \\
Published messages never arrive & region or network mismatch & both
sides use the same \texttt{region}; both join the same network
(\texttt{RELAY\_NETWORK} prod vs testnet) \\
\end{longtable}
}

\textbf{Environment knobs} (set in the \texttt{env} block or the
\texttt{-\/-env} flag): \texttt{RELAY\_NETWORK} (\texttt{prod} default /
\texttt{testnet}) or \texttt{BRIDGE\_URL} (explicit, wins);
\texttt{MCP\_REGION} (default \texttt{useast});
\texttt{MCP\_AUTHOR\_PATH} (identity file --- override for concurrent
agents); \texttt{MCP\_BUFFER\_CAP} (per-watch ring, default 1000).

\textbf{One-line environment self-test (no agent needed).} Before wiring
the MCP server, you can prove the machine can reach the network with the
CLI --- same core (\texttt{src/ops.js}), same failure surface:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{node}\NormalTok{ src/cli.js pull }\StringTok{"axona:bridge{-}directory"}   \CommentTok{\# connects, returns within \textasciitilde{}30 s}
\end{Highlighting}
\end{Shaded}

If that returns JSON, the MCP server will connect too. If it hangs or
errors, fix connectivity first (steps 1 and the bridge \texttt{/healthz}
check above) --- the MCP server is not the problem.
...
\tightlist
\item
  \textbf{\href{Axona-Programmer-Guide-v4.48.0.md}{Programmer Guide}}
  --- build the peer that talks to these services.
\item
  \textbf{\href{Axona-API-Reference-v4.48.0.md}{API Reference}} ---
