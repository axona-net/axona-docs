...
`@axona/protocol` **v4.40.0** API (kernel 4.40.0). One Node process connects
...
> (`wss://testnet.axona.net`) tracks the newest release — 4.40.0, which this
...
- [API Reference](Axona-API-Reference-v4.40.0.md) — every exported symbol.
- [AI Grounding](Axona-AI-Grounding-v4.40.0.md) — building with an AI assistant? Hand it this file.
...
npm install github:axona-net/axona-protocol#v4.40.0
```
...
```js
import { connect, deriveTopicId, KERNEL_VERSION } from '@axona/protocol';

const BRIDGE = 'wss://testnet.axona.net';           // live testnet bridge (kernel 4.40.0)
...
| Look up a specific symbol | [API Reference](Axona-API-Reference-v4.40.0.md) |
...
bridge runs kernel 4.40.0 (wire 4.0); install
`github:axona-net/axona-protocol#v4.40.0`. Production runs the same wire-4 line
