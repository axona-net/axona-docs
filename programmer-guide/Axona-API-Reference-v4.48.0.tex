...
\vspace{2em}
{\small The Axona API Reference \quad v4.48.0 \quad 2026-07-27\par}
\end{center}
\vfill
\null
\newpage
...
kernel --- v4.48.0, the version this reference describes.
...
Install \texttt{github:axona-net/axona-protocol\#v4.48.0}.
\end{quote}
...
\tightlist
\item
  \href{Quick-Start-v4.48.0.md}{Quick Start} --- 5-minute working
  roundtrip.
\item
  \href{Axona-Programmer-Guide-v4.38.0.md}{Programmer Guide} --- mental
...
assistant? Hand it the \href{Axona-AI-Grounding-v4.48.0.md}{AI
...
\begin{quote}
\textbf{\texttt{ts} is publish time, and it is the only legitimate input
to recency logic.} A subscription with \texttt{since} replays history,
so an envelope that \emph{arrives} now may have been \emph{published}
hours ago --- ``last seen'' / online / freshness features must compare
against \texttt{env.ts}, never the local clock at delivery, and must
keep the latest \texttt{ts} per author (replay order is not guaranteed).
Field bug: arrival-time stamping resurrected a full day of departed
users as ``online'' on every app launch.
\end{quote}
...
\subsubsection{\texorpdfstring{\texttt{connect(options)} →
\texttt{Promise\textless{}\{\ peer,\ author,\ nodeIdentity,\ transport,\ status,\ disconnect\ \}\textgreater{}}
\emph{(kernel 4.16.0; barrel export
4.48.0)}}{connect(options) → Promise\textless\{ peer, author, nodeIdentity, transport, status, disconnect \}\textgreater{} (kernel 4.16.0; barrel export 4.48.0)}}\label{connectoptions-promise-peer-author-nodeidentity-transport-status-disconnect-kernel-4.16.0-barrel-export-4.48.0}

\textbf{The single bootstrap an application uses.} \texttt{connect()}
composes the whole startup sequence --- mints the connection identity
(and, by default, an ephemeral author), builds the web transport +
\texttt{NeuronNode} + \texttt{AxonaDomain} + \texttt{AxonaPeer},
\texttt{start()}s, awaits \texttt{peer.ready()}, \textbf{and
self-integrates the node into the routing mesh}
(\texttt{peer.integrate()}) --- in the one correct order. It is exported
from the main barrel (and still from the \texttt{connect.js} subpath):

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ \{ connect \} }\ImportTok{from} \StringTok{\textquotesingle{}@axona/protocol\textquotesingle{}}\OperatorTok{;}          \CommentTok{// also: \textquotesingle{}@axona/protocol/connect.js\textquotesingle{}}
...
Every startup variation is an \textbf{option}, never a reason to drop to
...
\textbf{string} → durable author via \texttt{persistAs}; an author
identity → used as-is; \texttt{false} → no author minted. \\
...
\texttt{null}). \\
\texttt{transport} & \texttt{Transport} & Inject a custom/sim transport;
the web transport is only loaded when this is absent. \\
\texttt{nodeIdentity} & \texttt{Identity} & Inject a durable/pre-minted
connection key instead of minting one from \texttt{location}. \\
\texttt{domain} & \texttt{AxonaDomain} & Share \textbf{one} routing
domain (mesh) across several peers built in the same process. \\
\texttt{persist} & \texttt{PersistenceAdapter} & Enable
auto-checkpointing of identity, subscriptions, synaptome, and hosting
state. \\
\texttt{rootReplicas} & number & Replication factor for a topic's
authoritative cohort (advanced tuning). \\
\texttt{maxPublishBytes} & number & Per-publish cap (clamped to the
WebRTC-interop floor; advanced tuning). \\
\texttt{synaptomeMaintain} &
\texttt{boolean\ \textbackslash{}\textbar{}\ object} & Opt into
continuous near-quota maintenance (see \texttt{AxonaPeer} below). \\
...
\begin{quote}
\textbf{Why not assemble by hand?} The constructors and lifecycle calls
below are the \textbf{advanced building blocks \texttt{connect()}
composes} --- documented here for transport authors, sims, and tests,
not as an application setup path. Hand-assembling a peer is how apps
silently \textbf{skipped \texttt{peer.integrate()}} and so self-rooted
their topics as \textbf{singletons}, losing cross-region delivery (a
real shipped bug). If you build a peer by hand you MUST call
\texttt{peer.integrate()} after \texttt{start()}/\texttt{ready()} ---
\texttt{connect()} does it for you.
\end{quote}

\texttt{AxonaPeer} is the per-node DHT contract implementation --- one
instance per running node.

\subsubsection{\texorpdfstring{\texttt{new\ AxonaPeer(\{\ nodeIdentity,\ domain,\ transport,\ ...\ \})}
\emph{(advanced --- \texttt{connect()} builds
this)}}{new AxonaPeer(\{ nodeIdentity, domain, transport, ... \}) (advanced --- connect() builds this)}}\label{new-axonapeer-nodeidentity-domain-transport-...-advanced-connect-builds-this}
...
\ControlFlowTok{await}\NormalTok{ peer}\OperatorTok{.}\FunctionTok{integrate}\NormalTok{()}\OperatorTok{;}                     \CommentTok{// REQUIRED in a hand{-}driven lifecycle (connect() does it)}
\end{Highlighting}
\end{Shaded}

\subsubsection{\texorpdfstring{\texttt{peer.integrate()} →
\texttt{Promise\textless{}void\textgreater{}}}{peer.integrate() → Promise\textless void\textgreater{}}}\label{peer.integrate-promisevoid}

Actively integrate a freshly-started node into the routing mesh ---
\textbf{directed integration}: the node routes a small burst of traffic
at its XOR-neighbours so \emph{their} routing tables learn it, instead
of waiting for passive inbound adoption. Call it once after
\texttt{start()}/\texttt{ready()}. \texttt{connect()} calls it for you;
a hand-driven lifecycle \textbf{must} call it itself. Skipping it leaves
the node at the passive-adoption churn floor --- the failure that made
hand-assembled peers self-root their own topics as \textbf{singletons}
(no cross-region delivery). Idempotent; a no-op on a node
\texttt{connect()} already integrated.
...
sentinel to publish unsigned. There is no default author. Authorship is
\textbf{per call}: one peer serves any number of authors/personas by
varying \texttt{signWith} --- switching the active user of an app must
never reconnect the peer. \\
\end{longtable}
}
...
Takes a \textbf{descriptor or a 66-hex topic ID} --- exactly what
\texttt{sub} takes, so whoever can subscribe by a shared read handle can
unsubscribe by it (kernel 4.85.0; before that only the descriptor form was
accepted, GH \#64).
...
\NormalTok{  publishes}\OperatorTok{:}     \DataTypeTok{number}\OperatorTok{;}       \CommentTok{// (v4.41.0) advisory throughput: distinct messages ever cached at this}
                               \CommentTok{//   root (real messages, NOT kills); monotonic, maxed across the cohort.}
                               \CommentTok{//   On a peer/root running \textless{}4.41.0 this is still 0.}
...
\begin{quote}
\textbf{A real zero vs.~``no data yet'' --- read \texttt{stale}, not the
counters.} The numbers alone can't tell you which you have.
\texttt{stale:\ true} (⟺ \texttt{cohortSize:\ 0}, \texttt{ts:\ null},
\texttt{signer:\ null}) means \textbf{no snapshot arrived} in the window
--- every field reads 0 only because nothing reported, \emph{not}
because the topic is empty. This is exactly what you get calling
\texttt{metrics()} immediately after \texttt{pub()}: the first snapshot
rides a demand-driven lease and may land a beat later, so the
synchronous read races it. A result with \texttt{stale:\ false},
\texttt{cohortSize\ ≥\ 1}, and a real \texttt{ts}/\texttt{signer} is a
\textbf{genuine} reading --- its zeros are true current values
(\texttt{current\_count:\ 0} = the cache really is empty;
\texttt{bytes:\ 0} at a just-promoted root = nothing tallied there yet).
\textbf{Never read a \texttt{stale} result as ``zero activity.''} For a
live view, don't poll right after publishing --- keep a standing
\texttt{sub(metricTopic(T),\ …)} and treat metrics as the stream it is.
\end{quote}
...
\begin{quote}
\textbf{Common mistake (seen in the field):}
\texttt{peer.kill(msgId,\ \{\ signWith\ \})} --- omitting the topic. A
kill routes to the topic's roots, so it needs the descriptor exactly
like \texttt{pub}/\texttt{sub} do; there is no id-only form. The
...
\tightlist
\item
  \textbf{Max messages} --- up to \texttt{CACHE\_MAX} (\textbf{1024})
  per topic, also bounded by a 16 MB byte ceiling on the cache. When
  full, the lowest-ordered message (by signed \texttt{seq}) is evicted
  deterministically. (Configurable per peer via the
  \texttt{replayCacheSize} constructor option; a value of 1 is a
  \emph{retained/latest-value} slot --- pair it with
  \texttt{peer.pull(null,\ …)}.)
\item
...
\CommentTok{//   admission: \{                       // kernel 4.46.0+}
\CommentTok{//     seated, saturated, refusals,}
\CommentTok{//     capacity: \{                      // kernel 4.47.0+ — MEASURED, not counted}
\CommentTok{//       roles, overdue, overdueFrac, unserviced, worstAgeMs,}
\CommentTok{//       servicePressure,               // worstAgeMs / DROP\_MS}
\CommentTok{//       tickLagMs, tickLagMaxMs, tickDurMs, tickStalls,}
\CommentTok{//       helloPressure,                 // tickLag / HELLO\_DEADLINE\_MS}
\CommentTok{//     \},}
\CommentTok{//   \},}
\CommentTok{// \}}
\end{Highlighting}
...
\texttt{admission} reports whether this node will accept new roles, and
why not. \texttt{seated} is false during the post-join grace period;
\texttt{saturated} is true when the node is at capacity.

\textbf{\texttt{capacity} is measured, never counted.} \texttt{roles} is
inventory, not a limit --- production nodes routinely run several
hundred. The two numbers that mean something are the \emph{pressures},
each a ratio against a real protocol deadline:
...
\bottomrule\noalign{}
\endlastfoot
\texttt{servicePressure} & age of the least-recently-serviced role ÷
\texttt{DROP\_MS} & ≥ 0.6 \\
\texttt{helloPressure} & observed tick lag ÷
\texttt{HELLO\_DEADLINE\_MS} & ≥ 0.6 \\
\end{longtable}
}

A node holding 400 roles that services all of them on time is
\textbf{healthy}. A node holding 40 that cannot is \textbf{saturated}.
Poll the pressures, not \texttt{roles.length} --- an earlier release
used a role \emph{count} as the predicate and it was meaningless,
because the ceiling was exceeded twelvefold in ordinary operation.

\textbf{Refusal is normal, and refusal is not loss.} A node that
declines a role logs it; a declined message is forwarded to a node that
can take it. If there is no such node the message stops and is logged
\texttt{undeliverable} rather than being retried forever --- see the
Services Guide on the bridge fence for the one case where refusal is
absolute.
...
\KeywordTok{const}\NormalTok{ transport }\OperatorTok{=} \FunctionTok{webTransport}\NormalTok{(\{}
  ...
  \DataTypeTok{peerVersion}\OperatorTok{:} \StringTok{\textquotesingle{}4.48.0\textquotesingle{}}\OperatorTok{,}                   \CommentTok{// your app version (gated by the bridge)}
...
\NormalTok{KERNEL\_VERSION       }\CommentTok{// \textquotesingle{}4.48.0\textquotesingle{}   — kernel semver (npm release tag)}
...
\subsubsection{What changed in v4.39.0--v4.48.0 (2026-07-24; v4.48.0 on
testnet)}\label{what-changed-in-v4.39.0v4.48.0-2026-07-24-v4.48.0-on-testnet}

\textbf{No new API --- a bootstrap consolidation.} \texttt{connect()} is
now the single documented way to bring up a peer: it self-integrates the
node (\texttt{peer.integrate()}) as part of startup, and it accepts
every option an app could need (\texttt{bridge}, \texttt{location},
\texttt{author}, \texttt{k}, \texttt{ready}, \texttt{transport},
\texttt{nodeIdentity}, \texttt{domain}, \texttt{persist},
\texttt{rootReplicas}, \texttt{maxPublishBytes},
\texttt{synaptomeMaintain}, \texttt{web}) so no application ever drops
to the constructors. \texttt{connect} is now also exported from the
\textbf{main barrel}
(\texttt{import\ \{\ connect\ \}\ from\ \textquotesingle{}@axona/protocol\textquotesingle{}}),
not only the \texttt{connect.js} subpath. The constructors +
\texttt{peer.start}/\texttt{join}/\texttt{ready}/\texttt{integrate} are
reclassified as advanced building blocks \texttt{connect()} composes ---
hand-assembling a peer \emph{without} \texttt{peer.integrate()} was how
apps self-rooted their topics as singletons; that integration step is no
longer easy to miss. Existing signatures are unchanged.

\subsubsection{What changed in v4.28.0--v4.38.0 (2026-07-23; v4.38.0 on
testnet and
production)}\label{what-changed-in-v4.28.0v4.38.0-2026-07-23-v4.38.0-on-testnet-and-production}

\textbf{One additive API} --- \texttt{setAuthorClass} /
\texttt{getAuthorClass} --- everything else is behavioral: read-path
resilience, bridge independence, and departure correctness, each pinned
by a deterministic test and a production-shaped soak. Existing
signatures are unchanged.

\begin{itemize}
\item
  \textbf{Read-repair (v4.30.0--v4.36.0).} A read now survives a
  topic-closest node that is reachable-but-degraded or gone: the request
  escalates past dead or degraded holders, and a stuck subscriber
  repairs from the cohort. \texttt{pull(msgId)} is still \emph{exact}
  and pull-latest still \emph{eventually-consistent} --- you simply get
  an answer more often under churn. No call-site change.
\item
  \textbf{Leave-order handoff (v4.29.0).} A departing root now hands off
  its history \emph{before} it announces the departure (was the reverse,
  which dropped the hand-off). Clean \texttt{leave()}s no longer lose
  the tail of a topic's history.
\item
  \textbf{Bridge independence + vitality graduation (v4.37.0--v4.38.0).}
  When the signaling bridge graduates an established peer to free a
  slot, that peer keeps its WebRTC mesh instead of collapsing to zero. A
  third-party \texttt{peer-left} can no longer evict a peer from another
  peer's mesh; only a peer's own authenticated departure or its
  channel's own vitality retires it. The bridge now graduates the
  best-meshed node it can most afford to lose. Transparent to
  \texttt{peer.*} callers.
\item
  \textbf{Signed author-class (additive, \texttt{setAuthorClass} /
  \texttt{getAuthorClass}).} A voluntary, signed provenance claim bound
  to an \textbf{author} key ---
  \texttt{\textquotesingle{}human\textquotesingle{}},
  \texttt{\textquotesingle{}agent\textquotesingle{}},
  \texttt{\textquotesingle{}service\textquotesingle{}},
  \texttt{\textquotesingle{}bridge\textquotesingle{}}, or
  \texttt{\textquotesingle{}relay\textquotesingle{}}. It is \textbf{not
  a gate}: the kernel never reads it before routing, and its absence
  resolves to \texttt{\textquotesingle{}unstated\textquotesingle{}},
  never a default. Carried as a self-signed attestation on the author's
  owner-only profile topic, so it is discoverable from an Author ID
  alone.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{peer}\OperatorTok{.}\FunctionTok{setAuthorClass}\NormalTok{(cls}\OperatorTok{,}\NormalTok{ \{ signWith}\OperatorTok{,}\NormalTok{ operator}\OperatorTok{?,}\NormalTok{ operatorSignWith}\OperatorTok{?,}\NormalTok{ label}\OperatorTok{?}\NormalTok{ \})}
\NormalTok{  → }\BuiltInTok{Promise}\OperatorTok{\textless{}}\NormalTok{\{ msgId}\OperatorTok{,}\NormalTok{ attestation \}}\OperatorTok{\textgreater{}}
\CommentTok{//   cls: \textquotesingle{}human\textquotesingle{} | \textquotesingle{}agent\textquotesingle{} | \textquotesingle{}service\textquotesingle{} | \textquotesingle{}bridge\textquotesingle{} | \textquotesingle{}relay\textquotesingle{}}
\CommentTok{//   signWith: the AuthorIdentity to declare for and sign with.}

\NormalTok{peer}\OperatorTok{.}\FunctionTok{getAuthorClass}\NormalTok{(authorId}\OperatorTok{,}\NormalTok{ \{ timeoutMs }\OperatorTok{=} \DecValTok{1000}\NormalTok{ \})}
\NormalTok{  → }\BuiltInTok{Promise}\OperatorTok{\textless{}}\NormalTok{\{ }\KeywordTok{class}\OperatorTok{,}\NormalTok{ operator}\OperatorTok{,}\NormalTok{ operatorVerified}\OperatorTok{,}\NormalTok{ label}\OperatorTok{?,}\NormalTok{ ts}\OperatorTok{?}\NormalTok{ \}}\OperatorTok{\textgreater{}}
\CommentTok{//   authorId: 64{-}hex Author ID (== signerPubkey). Pulls + verifies the}
\CommentTok{//   author\textquotesingle{}s attestation; any missing/invalid/unparseable result is}
\CommentTok{//   \{ class: \textquotesingle{}unstated\textquotesingle{} \} — never a wrong default.}
\end{Highlighting}
\end{Shaded}

  Apps that badge or filter by human/agent should resolve via
  \texttt{getAuthorClass} (keyed on the authenticated
  \texttt{signerPubkey}), not on any in-body field, and render
  \texttt{\textquotesingle{}unstated\textquotesingle{}} normally rather
  than hiding it.
\end{itemize}

\subsubsection{What changed in v4.23.0--v4.27.1 (2026-07-17 roll;
v4.27.1 promoted to
production)}\label{what-changed-in-v4.23.0v4.27.1-2026-07-17-roll-v4.27.1-promoted-to-production}

\textbf{No API change} --- the whole span is internal: region naming,
role lifecycle, and load hardening.

\begin{itemize}
\tightlist
\item
  \textbf{Canonical regions (v4.23.0).} Ocean/uninhabited region codes
  fold to their nearest populated cell (\texttt{canonicalRegion}), and
  regions carry human-readable animal names. Descriptor-visible behavior
  unchanged --- any region you could name before still resolves.
\item
  \textbf{Acked leave-handoff (v4.24.0--v4.24.1).} A departing root's
  history hand-off is now acknowledged and retried (was
  fire-and-forget), with a cohort-spray fallback.
\item
  \textbf{Explicit role natures (v4.25.0--v4.26.0).} Internal refactor
  phases: a node's per-topic role (ROOT / CHILD / BACKUP / HOLDER) is
  explicit and audited, and per-node role state is bounded (backup
  eviction).
\item
  \textbf{Join-storm hardening (v4.27.0--v4.27.1).} A (re)joining relay
  is seeded with role state gradually (sender pacing + bounded
  time-sliced ingest) and a node whose mesh dissolves re-bootstraps from
  the bridge. Fixes the production failure where a rejoining
  infrastructure node was firehosed into event-loop starvation and
  evicted. Invariant I-11: \emph{bulk work never starves liveness.}
\end{itemize}
...
\tightlist
\item
  \textbf{\texttt{connect()} (v4.16.0; barrel export + single-bootstrap
  consolidation v4.48.0)} --- the one-call bootstrap above; no other API
  change.
\item
