...
\emph{(kernel 4.16.0 · testnet --- versioned with the protocol it
describes)}
...
\item
  ...
  4.16.0 (\texttt{KERNEL\_VERSION})
\item
  \textbf{Live network}: the 4.x line runs on \textbf{testnet} ---
  \texttt{wss://testnet.axona.net}. The \textbf{production} federated
  pair (\texttt{wss://bridge.axona.net} east +
  \texttt{wss://bridge-west.axona.net} west) is still on the 3.x line;
  the two don't interoperate (the wire major partitions them). Run 4.x
  services against testnet --- point relays/MCP at it explicitly (their
...
\item
  ...
  \tightlist
  \item
    \href{Quick-Start-v4.16.0.md}{Quick Start} ·
    \href{Axona-Programmer-Guide-v4.16.0.md}{Programmer Guide} ·
    \href{Axona-API-Reference-v4.16.0.md}{API Reference} --- the
    library.
  \item
    \href{Axona-AI-Grounding-v4.16.0.md}{AI Grounding} --- building with
    an AI assistant.
  \item
...
\ExtensionTok{curl}\NormalTok{ https://bridge.example.net/healthz     }\CommentTok{\# → \{"status":"ok","version":…,"kernelVersion":"4.16.0"\}}
\end{Highlighting}
\end{Shaded}
...
\item
  \textbf{Metrics} --- every \textasciitilde20 s the relay walks its
  rooted topics and publishes a signed snapshot
  (\texttt{\{current\_count,\ seq,\ subscribers,\ bytes\}}) to the
  derived \texttt{metricTopic(T)}, so clients can \texttt{sub()} for
  live counts + a rolling \textasciitilde48 h trend instead of polling.
  As of v4.3.0 \textbf{both open and owned} topics publish metrics
  (owned-topic activity counts are public too); \texttt{seq} is the
  dense message counter (total events ever, kills included). Because
...
\texttt{prod} (\texttt{bridge.axona.net}, \textbf{3.x}) or
\texttt{testnet} (\texttt{testnet.axona.net}, \textbf{4.x}). A v4.16.0
relay must use \texttt{testnet} --- the code default \texttt{prod} is
the 3.x line and rejects a 4.x peer at the handshake. \\
...
targets \textbf{production} (the 3.x line); a v4.16.0 MCP peer must set
\texttt{RELAY\_NETWORK=testnet} to join the 4.x line (the two don't
interoperate). Within a line, publishing interoperates with the live
apps on that line.
...
the live one for your line --- \texttt{testnet.axona.net} for 4.x,
\texttt{bridge.axona.net} for 3.x) \\
Keep a topic's messages alive when no author is online & A
\textbf{relay} that \texttt{host()}s it \\
Publish live subscriber counts for a topic & A \textbf{relay} (metrics
on --- the default) \\
...
\tightlist
\item
  \textbf{\href{Axona-Programmer-Guide-v4.16.0.md}{Programmer Guide}}
  --- build the peer that talks to these services.
\item
  \textbf{\href{Axona-API-Reference-v4.16.0.md}{API Reference}} ---
