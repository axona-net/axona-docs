...
\vspace{2em}
{\small The Axona API Reference \quad v4.40.0 \quad 2026-07-24\par}
\end{center}
\vfill
\null
\newpage
...
kernel --- v4.40.0, the version this reference describes.
...
Install \texttt{github:axona-net/axona-protocol\#v4.40.0}.
\end{quote}
...
\tightlist
\item
  \href{Quick-Start-v4.40.0.md}{Quick Start} --- 5-minute working
  roundtrip.
\item
...
assistant? Hand it the \href{Axona-AI-Grounding-v4.40.0.md}{AI
...
4.40.0)}}{connect(options) → Promise\textless\{ peer, author, nodeIdentity, transport, status, disconnect \}\textgreater{} (kernel 4.16.0; barrel export 4.40.0)}}\label{connect-options-promise-peer-author-nodeidentity-transport-status-disconnect-kernel-4.16.0-barrel-export-4.40.0}

\textbf{The single bootstrap an application uses.} \texttt{connect()}
composes the whole startup sequence --- mints the connection identity (and,
by default, an ephemeral author), builds the web transport +
\texttt{NeuronNode} + \texttt{AxonaDomain} + \texttt{AxonaPeer},
\texttt{start()}s, awaits \texttt{peer.ready()}, \textbf{and self-integrates
the node into the routing mesh} (\texttt{peer.integrate()}) --- in the one
correct order. It is exported from the main barrel (and still from the
\texttt{connect.js} subpath):
...
Every startup variation is an \textbf{option}, never a reason to drop to the
constructors below:
...
\texttt{domain} & \texttt{AxonaDomain} & Share \textbf{one} routing domain
(mesh) across several peers built in the same process. \\
\texttt{persist} & \texttt{PersistenceAdapter} & Enable auto-checkpointing
of identity, subscriptions, synaptome, and hosting state. \\
...
\texttt{synaptomeMaintain} & \texttt{boolean} \textbar{} object & Opt into
...
below are the \textbf{advanced building blocks \texttt{connect()} composes}
--- documented here for transport authors, sims, and tests, not as an
application setup path. Hand-assembling a peer is how apps silently
\textbf{skipped \texttt{peer.integrate()}} and so self-rooted their topics
as \textbf{singletons}, losing cross-region delivery (a real shipped bug).
If you build a peer by hand you MUST call \texttt{peer.integrate()} after
\texttt{start()}/\texttt{ready()} --- \texttt{connect()} does it for you.
\end{quote}

\texttt{AxonaPeer} is the per-node DHT contract implementation --- one
instance per running node.

\subsubsection{\texorpdfstring{\texttt{new\ AxonaPeer(\{\ nodeIdentity,\ domain,\ transport,\ ...\ \})} \emph{(advanced --- \texttt{connect()} builds this)}}{new AxonaPeer(\{ nodeIdentity, domain, transport, ... \}) (advanced — connect() builds this)}}\label{new-axonapeer-nodeidentity-domain-transport-...}
...
\textbf{directed integration}: the node routes a small burst of traffic at
its XOR-neighbours so \emph{their} routing tables learn it, instead of
waiting for passive inbound adoption. Call it once after
\texttt{start()}/\texttt{ready()}. \texttt{connect()} calls it for you; a
hand-driven lifecycle \textbf{must} call it itself. Skipping it leaves the
node at the passive-adoption churn floor --- the failure that made
hand-assembled peers self-root their own topics as \textbf{singletons} (no
cross-region delivery). Idempotent; a no-op on a node \texttt{connect()}
already integrated.
...
\NormalTok{  publishes}\OperatorTok{:}     \DataTypeTok{number}\OperatorTok{;}       \CommentTok{// (v4.41.0) advisory throughput: distinct messages ever cached (excl. kills); maxed across cohort}
...
\texttt{signer:\ null}) means \textbf{no snapshot arrived} in the window --- every
field reads 0 only because nothing reported, \emph{not} because the topic is empty.
This is exactly what you get calling \texttt{metrics()} immediately after
\texttt{pub()}: the first snapshot rides a demand-driven lease and may land a beat
later, so the synchronous read races it. A result with \texttt{stale:\ false},
\texttt{cohortSize\ ≥\ 1}, and a real \texttt{ts}/\texttt{signer} is a
\textbf{genuine} reading --- its zeros are true current values
(\texttt{current\_count:\ 0} = the cache really is empty; \texttt{bytes:\ 0} at a
just-promoted root = nothing tallied there yet). \textbf{Never read a
\texttt{stale} result as ``zero activity.''} For a live view, don't poll right
after publishing --- keep a standing \texttt{sub(metricTopic(T),\ …)} and treat
metrics as the stream it is.
\end{quote}
...
\KeywordTok{const}\NormalTok{ transport }\OperatorTok{=} \FunctionTok{webTransport}\NormalTok{(\{}
  ...
  \DataTypeTok{peerVersion}\OperatorTok{:} \StringTok{\textquotesingle{}4.40.0\textquotesingle{}}\OperatorTok{,}                   \CommentTok{// your app version (gated by the bridge)}
...
\NormalTok{KERNEL\_VERSION       }\CommentTok{// \textquotesingle{}4.40.0\textquotesingle{}   — kernel semver (npm release tag)}
...
\subsubsection{What changed in v4.39.0--v4.40.0 (2026-07-24; v4.40.0 on
testnet)}\label{what-changed-in-v4.39.0v4.40.0-2026-07-24-v4.40.0-on-testnet}

\textbf{No new API --- a bootstrap consolidation.} \texttt{connect()} is now
the single documented way to bring up a peer: it self-integrates the node
(\texttt{peer.integrate()}) as part of startup, and it accepts every option
an app could need (\texttt{bridge}, \texttt{location}, \texttt{author},
\texttt{k}, \texttt{ready}, \texttt{transport}, \texttt{nodeIdentity},
\texttt{domain}, \texttt{persist}, \texttt{rootReplicas},
\texttt{maxPublishBytes}, \texttt{synaptomeMaintain}, \texttt{web}) so no
application ever drops to the constructors. \texttt{connect} is now also
exported from the \textbf{main barrel}
...
hand-assembling a peer \emph{without} \texttt{peer.integrate()} was how apps
self-rooted their topics as singletons; that integration step is no longer
easy to miss. Existing signatures are unchanged.
...
\item
  ...
  consolidation v4.40.0)} --- the one-call bootstrap above; no other API
  change.
\item
