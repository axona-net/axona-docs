...
- **[`architecture/`](architecture/)** — the full-stack architecture note: kernel · protocol · transport · bridge, the wire protocol, the complete application API, and the demo-app source. [PDF](architecture/Axona%20Architecture%20v0.7.10.pdf) · [`.tex`](architecture/Axona-Architecture.tex). Also the [Axona vs. Vivaldi](architecture/Axona-vs-Vivaldi-v0.1.md) design note (measured-RTT keyspace router vs. predictive latency-coordinate embedding) and the [E-1 placement-defense decision record](architecture/E-1-Placement-Defense-v0.1.md) (memory-hard PoW selected, RTT/Vivaldi rejected because the prefix is a routing area-code; includes the impact on the publishID/transportID split).
