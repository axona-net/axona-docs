...
- **[`architecture/`](architecture/)** — the full-stack architecture note: kernel · protocol · transport · bridge, the wire protocol, the complete application API, and the demo-app source. [PDF](architecture/Axona%20Architecture%20v0.7.10.pdf) · [`.tex`](architecture/Axona-Architecture.tex). Also the [Axona vs. Vivaldi](architecture/Axona-vs-Vivaldi-v0.1.md) design note (measured-RTT keyspace router vs. predictive latency-coordinate embedding; bears on the E-1 PoW-vs-Vivaldi decision).
