...
\begin{quote}
\textbf{Which network?} The 4.x line runs on \textbf{testnet}
(\texttt{wss://testnet.axona.net}) during the current phase; the
\textbf{production} bridges (\texttt{wss://bridge.axona.net}) are still
on the 3.x line. The wire major partitions them (wire 3.0 vs 4.0), so a
v4.x peer talks to testnet, a v3.x peer talks to prod --- they do not
interoperate. Install \texttt{github:axona-net/axona-protocol\#v4.16.0}
and point at testnet to use this API surface.
\end{quote}
...
\tightlist
\item
  \href{Quick-Start-v4.16.0.md}{Quick Start} --- 5-minute working
  roundtrip.
\item
  \href{Axona-Programmer-Guide-v4.16.0.md}{Programmer Guide} --- mental
...
assistant? Hand it the \href{Axona-AI-Grounding-v4.16.0.md}{AI
...
land. You do not tune any of this.
\end{quote}
...
\textbf{Nearest-replica reads (v4.11.1--v4.16.0).} A pull is answered by
the \textbf{first replica the request reaches} --- a cohort member, a
child relay, or a \texttt{host()} node --- not necessarily the topic's
root. This lowers latency, spreads reads off the root, and lets a pull
that would otherwise strand toward the root be served by any
cache-holder it passes. Two consistency tiers:
...
\begin{quote}
\textbf{Cold-topic gotcha.} Because publication starts \emph{on demand},
the very first snapshot for a topic nobody was watching arrives
\textbf{\textasciitilde2--20 s after} demand turns on (the root's next
refresh tick, plus routing) --- longer than the default 1500 ms
collection window. So the \textbf{first \texttt{metrics()} call on a
cold topic normally returns \texttt{stale:\ true}}. That is not an
error: either call it again a few seconds later, pass a longer window
(\texttt{\{\ timeoutMs:\ 25\_000\ \}} spans one full publish cadence),
or --- better --- keep a standing \texttt{sub(metricTopic(T),\ …)} and
treat metrics as the stream it is. If \emph{any} peer watched the
topic's metrics within the 48 h hold window, replay serves the last
cached snapshot immediately and the first call succeeds.
\end{quote}
...
\textbf{When snapshots arrive.} Metrics is an \emph{eventually-arriving
stream}, not a synchronous read --- plan around this timing contract:

\begin{itemize}
\tightlist
\item
  \textbf{First snapshot:} typically \textbf{\textasciitilde2 s} after
  you subscribe (measured on the testnet), but allow \textbf{up to
  \textasciitilde20--25 s} --- the root publishes on its next refresh
  tick, and a root that churns right then must re-home first. If any
  peer watched this topic's metrics within the last 48 h, replay hands
  you the most recent snapshot immediately.
\item
...
\item
  ...
  returns.
\item
  \textbf{Tests and short-lived probes:} subscribe early and \emph{await
  the first envelope} with a generous allowance (≥ 25 s) --- a
  subscription torn down a few seconds after it starts will often
  observe nothing and misread a healthy topic as ``metrics not
  working.''
\end{itemize}
...
\KeywordTok{const}\NormalTok{ transport }\OperatorTok{=} \FunctionTok{webTransport}\NormalTok{(\{}
  ...
  \DataTypeTok{peerVersion}\OperatorTok{:} \StringTok{\textquotesingle{}4.16.0\textquotesingle{}}\OperatorTok{,}                   \CommentTok{// your app version (gated by the bridge)}
...
\NormalTok{KERNEL\_VERSION       }\CommentTok{// \textquotesingle{}4.16.0\textquotesingle{}   — kernel semver (npm release tag)}
...
\subsubsection{What changed in v4.12.0--v4.16.0 (2026-07-02 testnet
roll)}\label{what-changed-in-v4.12.0v4.16.0-2026-07-02-testnet-roll}
...
\item
  ...
  is retired. Consequence: the first snapshot on a cold topic arrives
  \textasciitilde2--20 s \emph{after} the first metric subscribe --- see
  the timing contract in §4.3.
\item
...
\item
  \textbf{Nearest-replica reads (v4.11.1--v4.16.0).} \texttt{pull} is
  answered by the first replica the request reaches instead of always
  the root: exact for \texttt{pull(msgId)}, and \emph{recent}
