...
\textbf{Build apps that have no server.} \emph{(kernel 4.16.0 ·
testnet)}
...
\tightlist
\item
  \href{Quick-Start-v4.16.0.md}{Quick Start} --- a working roundtrip in
  5 minutes. Do it first.
\item
  \href{Axona-API-Reference-v4.16.0.md}{API Reference} --- every
  signature, exactly.
\item
  \href{Axona-AI-Grounding-v4.16.0.md}{AI Grounding} --- building with
  an AI assistant? Hand it this file.
\item
  \href{Axona-Services-Guide-v4.16.0.md}{Services Guide} --- running
  bridges and relays (you can skip this for a long time).
\end{itemize}

\begin{quote}
\textbf{Building with an AI?} Most Axona apps are. Paste
\url{Axona-AI-Grounding-v4.16.0.md} into your assistant's context and it
...
If you haven't run the \href{Quick-Start-v4.16.0.md}{Quick Start}, do it
...
switches them on, and the first snapshot lands \textbf{\textasciitilde2
s later in the typical case --- allow up to \textasciitilde25 s} (then
one every \textasciitilde20 s while you stay subscribed). Two
consequences worth knowing:

\begin{itemize}
\tightlist
\item
  The \textbf{first} \texttt{peer.metrics()} call on a topic nobody was
  watching usually returns \texttt{stale:\ true} --- its default
  collection window (1.5 s) closes before the first on-demand snapshot
  can arrive. Call it again, pass \texttt{\{\ timeoutMs:\ 25\_000\ \}},
  or just use the subscription form above.
\item
  \textbf{Don't subscribe-then-teardown in seconds} (test suites love to
  do this) and conclude metrics is broken --- keep the subscription open
  and await the first envelope with a real allowance.
\end{itemize}
...
\item
  ...
  \href{Axona-Services-Guide-v4.16.0.md}{Services Guide} is yours; not
  before.
\end{itemize}
...
\href{Axona-API-Reference-v4.16.0.md}{API Reference}, it didn't survive
v3 --- the reference is the source of truth.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\emph{Axona Programmer Guide · kernel 4.16.0 · testnet · 2026-07-02}
