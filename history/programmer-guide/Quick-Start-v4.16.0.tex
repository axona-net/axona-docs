...
current \texttt{@axona/protocol} \textbf{v4.16.0} API (kernel 4.16.0).
...
uses testnet + the \texttt{\#v4.16.0} pin. To target production instead,
...
\tightlist
\item
  \href{Axona-Programmer-Guide-v4.16.0.md}{Programmer Guide} --- the
...
\ExtensionTok{npm}\NormalTok{ install github:axona{-}net/axona{-}protocol\#v4.16.0}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{const}\NormalTok{ BRIDGE }\OperatorTok{=} \StringTok{\textquotesingle{}wss://testnet.axona.net\textquotesingle{}}\OperatorTok{;}           \CommentTok{// live testnet bridge (kernel 4.16.0)}
...
Look up a specific symbol & \href{Axona-API-Reference-v4.16.0.md}{API
Reference} \\
\end{longtable}
}
...
mismatch. The testnet bridge runs kernel 4.16.0 (wire 4.0); install
\texttt{github:axona-net/axona-protocol\#v4.16.0}. Note this also fires
