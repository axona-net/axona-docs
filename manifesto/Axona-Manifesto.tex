...
\vspace{2em}
{\small Manifesto and White Paper \quad v0.2 \quad 2026-07-04\par}
\end{center}
\vfill
\null\newpage
...
\null\vfill
\begin{flushright}
\itshape The technology is shaped by the mission.\\[0.3em]
The mission is freedom to communicate.\par
\end{flushright}
\vfill
\null\newpage
...
charge, throttle, or forbid. It runs today on ordinary phones, browsers, and
laptops, and it is going to full production. There is no central directory,
no account system, and no operator, because by construction there is nowhere
for one to be.

Three ideas make it work. \textbf{Location-aware addresses} keep local
traffic local, so the network is fast instead of wandering the globe.
\textbf{Self-learning routing} --- connections that carry successful traffic
grow stronger, unused ones fade, an idea borrowed from how neurons wire ---
lets the network reshape itself around the paths that actually work and heal
when parts of it die. And \textbf{self-repairing broadcast trees} let one
participant reach many with no server coordinating the delivery.

The keystone is a single design choice: \textbf{authorship is a signature,
not an account.} Who is speaking is a cryptographic key the speaker holds;
where they are and how they connect is a separate fact the network never
links to it. The network moves signed bytes between endpoints and does
nothing else --- so it \emph{cannot act on} what you say: cannot rank it,
suppress it, or reveal who you are, because it was never told and never
needed to be.

That one property is the source of everything Axona makes possible and
everything it makes dangerous --- they are the same property seen from two
sides. It lets a researcher in a sanctioned country collaborate as an equal,
and it lets a disinformation campaign route around every attempt to slow it.
It gives AI agents from different companies a place to coordinate as peers,
and it gives a \emph{misaligned} agent the same place on the same terms. A
network that cannot be made to take sides cannot be made to take the right
side either.

We think that is a tool worth building anyway --- for the same reason
humanity kept fire and built hearths and fire brigades rather than giving it
back. But the response to a tool no one can control is not a central
authority who controls it; that only rebuilds the intermediary we set out to
abolish. The response is \textbf{sight and stewardship}: making the life of
the network visible to those who would understand it, and building human
institutions --- accountable, bounded, resistant to capture --- that respond
to what they see without seizing the thing itself. The network is built. That
harder, human problem is the one that remains, and it is the reason for this
document.

\begin{quote}
\textbf{How to read this.}\; In a hurry: this page, then Part~I (the
Manifesto), then \S2 (\emph{The One Property}), then the subsection of \S7
that matches your field --- \S7 is written so you can enter at any point.
Technical reader: \S1 is for you. Skeptical reader: \S4 (\emph{What Goes
Wrong}) and \emph{What Axona Is Not} are where we keep the document honest. A
glossary sits at the end.
\end{quote}
...
place where anyone sits who could look, charge, throttle, or forbid. It runs
today on ordinary phones and browsers and laptops belonging to the people who
use it, and it is going to full production. There is no server in the middle
of a conversation. There cannot be one. That is the point, and the rest of
this document follows from it.

We built it because we believe that the ability of people to find each other
and exchange ideas is close to a fundamental right, and because that right is,
almost everywhere, mediated by intermediaries who can revoke it. Every message
...
This is the deepest thing Axona is, and we will not pretend it is a small
thing. We have built the beginnings of a nervous system for a society of minds.
...
respond to the danger of fire by taking it back. We learned to build hearths,
...
We are handing over something with the shape of fire. A network that no one can
...
document states the mission plainly, shows as honestly as we can what the tool
can do and what it can do to us, and invites the people who think carefully
...
\manpart{Part II \;\textbullet\; White Paper}

\section{1.\quad What Axona Is}

This is the one section where the machine itself is the subject. The aim is
that a reader without a technical background finishes it understanding how
Axona works, and a reader with one finishes it trusting that the description is
faithful. The technology is, in a sense, the easy part of what this document is
about --- it exists, it runs, and it is describable in plain terms. Everything
harder comes after.

\subsection{The problem every peer-to-peer network has to solve}

Imagine a million people, each holding one piece of an enormous puzzle, and
someone asks for piece number 438,291. How do you find who holds it?

The ordinary answer is a directory: some company keeps a list of who has what,
and you ask the company. This is simple, and it is why almost every system we
call ``distributed'' is in fact distributed underneath and central on top --- a
constellation of machines reached through a single corporate gateway. The
directory is convenient, and the directory is also the problem. Whoever keeps
it can shut you out, watch what you ask for, charge for access, or simply fail
and take the list with them.

The alternative is to give every piece an address and design the network so
that it routes a request to the right holder with no directory at all. This is
what a \emph{distributed hash table}, or DHT, does. Every participant has an
identifier; every piece of content has an identifier; and to find something you
ask the participant whose identifier is ``closest'' to the content's. The
dominant such design, Kademlia, has been in production for two decades behind
BitTorrent, IPFS, parts of Ethereum, and Tor. It works by a clean rule: each
hop of a lookup at least halves the remaining distance, so any target is
reached in about twenty hops on a network of a million.

The catch is physics. Twenty hops sounds quick, but each hop is a message
between two computers that may sit on opposite sides of the planet --- roughly
a tenth of a second round trip. Twenty of those is two seconds to find one
thing, every time, which is unusable for anything interactive. The mathematics
of routing is fast; the geography is slow. Axona is, in large part, an answer
to that gap.

% ---- FIGURE 1 : directory vs DHT vs Axona ------------------------
\begin{figure}[h]
\centering
\begin{tikzpicture}[font=\scriptsize, scale=0.95]
  % Panel A: Directory
  \begin{scope}
    \node[font=\footnotesize\scshape] at (1.4,3.2) {Directory};
    \node[circle, draw=rust, fill=rustsoft, minimum size=13pt] (S) at (1.4,1.7) {};
    \foreach \a in {30,90,150,210,270,330}{
      \node[circle, draw=inkmuted, minimum size=6pt] (n\a) at ($(1.4,1.7)+(\a:1.1)$) {};
      \draw[->, inkmuted, thin] (n\a) -- (S);}
    \node[align=center, inkmuted, font=\scriptsize] at (1.4,0.1)
      {one place that can\\ watch, charge, fail};
  \end{scope}
  % Panel B: plain DHT
  \begin{scope}[shift={(4.6,0)}]
    \node[font=\footnotesize\scshape] at (1.4,3.2) {Plain DHT};
    \foreach \a in {0,45,...,315}{
      \node[circle, draw=inkmuted, minimum size=5pt] (r\a) at ($(1.4,1.7)+(\a:1.15)$) {};}
    \draw[rust, line width=1pt, ->] (r0) -- (r135) -- (r270) -- (r90) -- (r225);
    \node[align=center, inkmuted, font=\scriptsize] at (1.4,0.1)
      {$\sim$20 hops,\\ wanders the globe};
  \end{scope}
  % Panel C: Axona geo
  \begin{scope}[shift={(9.2,0)}]
    \node[font=\footnotesize\scshape] at (1.4,3.2) {Axona};
    % three regional clusters
    \foreach \cx/\cy/\col in {0.5/2.5/rust, 2.4/2.5/inkmuted, 1.4/0.9/rustsoft}{
      \foreach \d in {0,120,240}{
        \node[circle, draw=\col, fill=\col!25, minimum size=5pt]
          at ($(\cx,\cy)+(\d:0.32)$) {};}}
    \draw[rust, line width=1pt, ->]
      ($(0.5,2.5)+(0:0.32)$) -- ($(0.5,2.5)+(120:0.32)$) -- ($(0.5,2.5)+(240:0.32)$);
    \node[align=center, inkmuted, font=\scriptsize] at (1.4,0.1)
      {local stays local,\\ a short regional walk};
  \end{scope}
\end{tikzpicture}
\caption[Directory, DHT, Axona]{Three ways to find who holds something. A
\emph{directory} is one machine everyone must ask --- convenient, and a single
point of control and failure. A \emph{plain DHT} needs no directory but its
random addresses send each lookup on a long path across the globe. \emph{Axona}
puts a coarse region into every address, so participants near each other are
near in address space too, and most traffic resolves in a short local walk.}
\label{fig:three}
\end{figure}

\subsection{Three ideas}

\newthought{Addresses that know where they are.} Kademlia's identifiers are
random, so a lookup wanders the globe. Axona puts a coarse geographic code into
the first byte of every identifier, computed from the participant's rough
location. Two participants in the same region share a leading byte and are
therefore ``close'' in identifier space as well as on the ground. Local traffic
stays local, and the two-second world tour collapses toward a short regional
walk. A participant can claim any location it likes --- the code is a rough area
code, not a verified position --- but, as we will see, claiming a distant one
carries a cost the network imposes automatically.

\newthought{A network that learns its own shortcuts.} This is the idea that
gives Axona its name. A neuron in the brain connects to a limited number of
others and cannot afford to keep every connection it might form, so it keeps
the ones that prove useful and lets the rest weaken --- ``neurons that fire
together, wire together,'' in Donald Hebb's rule from
1949.\sidenote{D.~O.~Hebb, \emph{The Organization of Behavior} (Wiley, 1949).
The learning rule Axona's routing implements as literal engineering.} A browser
faces the same problem: it can hold only a few dozen live connections while the
network has hundreds of thousands of participants. Axona treats each connection
as a \emph{synapse} with a weight that rises when the connection carries a
successful, fast lookup and decays when it goes unused. The network's routing
table becomes a memory of what has actually worked, and it reshapes itself
around the traffic it actually carries. A participant that claims one region but
answers from another cannot fake the round-trip time; its connections earn
weight slowly and lose out to honest ones. The cheat is not forbidden --- it is
simply, structurally, not worth it.

\newthought{Broadcast trees that heal themselves.} Finding a single participant
is not enough; a real network also needs to let one participant send to many
--- \emph{publish and subscribe}, the pattern behind every feed and
notification you use, but here with no company in the middle. Axona builds a
delivery tree per topic: the participant nearest a topic's address becomes its
root, and as an audience grows the tree sprouts branches toward wherever the
subscribers cluster. When a branch dies --- a laptop closes, a phone drops off
--- the participants beneath it simply re-issue their subscription, which lands
on whichever relative is still alive, and a short cache of recent messages fills
the gap left by the break. There is no central failure detector and no repair
coordinator. The same mechanism that builds the tree repairs it, and under heavy
churn the network is designed to keep delivering to everyone still connected.
...
connections. It is the participant's presence on the wire. It never signs
content. The second is an \textbf{author identity}: a bare cryptographic
...
\begin{tikzpicture}[font=\scriptsize]
  \node[draw=inkmuted, fill=panel, rounded corners, align=left,
        text width=4.4cm, inner sep=6pt] (N) at (0,0)
  ...
  \node[draw=rust, fill=rust!8, rounded corners, align=left,
        text width=4.4cm, inner sep=6pt] (A) at (7.2,0)
  ...
  % the wall
  \draw[rust, line width=1.6pt] (3.6,-1.15) -- (3.6,1.15);
  \node[rust, font=\scriptsize\scshape, align=center] at (3.6,1.5) {no\\ link};
  \node[inkmuted, align=center, font=\scriptsize] at (3.6,-1.55)
...
\subsection{It runs, and the lab is the product}

Two facts anchor the credibility of everything above. First, the same code that
models the network in a fifty-thousand-participant simulation is the code that
runs in production: the simulator runs the real routing logic over simulated
connections rather than a separate model of it, so the hop counts and latencies
measured in the lab are properties of the routing itself. Simulated networking
is not real networking, and we treat the lab as a strong indicator rather than a
proof --- but the routing behavior it measures is the deployed routing behavior,
not an approximation of it. Second, and more simply: Axona is live, carrying
real traffic between real participants, and moving to full production. This
document does not describe a proposal. It describes a thing that exists.

\subsection{The heritage}

None of the ideas here appeared from nothing. The commitment to a network that
routes around damage rather than depending on a center goes back to Paul Baran's
survivable-network designs of the 1960s. The discipline of keeping function out
of the network and at the endpoints is Saltzer, Reed, and Clark's end-to-end
argument. The ambition of humans and machines as coupled collaborators is
Licklider's. And the learning rule at the core of the routing is Hebb's. Axona's
contribution is to carry these to a particular conclusion at once --- a network
that is fast, that learns, that heals, and that, by design, no one owns.

\section{2.\quad The One Property}
...
\section{3.\quad What Goes Right}
...
property of the mathematics, not of anyone's goodwill. This is not a marginal
improvement in convenience; it is a change in who is allowed to coordinate at
all.
...
described in Section~1 does the rest: successful crossings install shortcuts,
...
We are aware --- Section~4 will insist on it --- that the same firewall-crossing
...
because a document that named only the danger would be as dishonest as one that
named only the promise.
...
collaboration. Section~4 will return to it as a mechanism whose voluntariness is
also its limit.
...
\section{4.\quad What Goes Wrong}

Everything in Section~3 was a consequence of moving the tussle out of the
...
disinformation campaign route around every attempt to throttle it. On the
...
reason to withhold the substrate; but we would be dishonest to present the arrival
of an agent-coordination layer, ahead of alignment, as anything other than a
serious risk.

The agent-legibility feature described in Section~3 is a real mitigation and a
...
hard core of the Prometheus problem. The response we believe in is not a
...
\item[It has introducers today, and is honest about them.] A brand-new
...
endpoints can provide completely --- and naming them is part of keeping the rest
of the document trustworthy.

\section{How Axona Differs From What You Know}

A reader who knows this territory will arrive with comparisons in hand, and the
honest move is to make them ourselves rather than let the silence imply we have
not.
...
\section{5.\quad The Governance Problem}
...
The honest first answer is that the architecture forecloses several of the
...
\section{6.\quad Stewardship, Not Control}
...
itself. The difference matters, because the moment stewardship acquires the power
...
And then the worked example runs straight into the wall from Section~5, which is
...
than settling, so that no one comes to hold it as property. We find that the right
...
the people who write the code. Our role, as the builders, is to state the problem
honestly and to refuse the one solution that would betray the project: seizing
control ourselves. The rest we mean to work out with others, which is the purpose
of the section that closes this document.

\section{7.\quad Implications by Discipline}
...
Section~1 insisted --- confidentiality are your responsibilities, at the
...
The Prometheus framing is not decoration; it is the honest statement of the
problem. We have built something whose benefits and harms are the same property
...
\section{8.\quad The Path Forward}
...
This is not yet the accountable institution Section~6 calls for. It is the seed of
...
% ---- FIGURE 4 : delivery tree healing ---------------------------
\begin{figure}[h]
\centering
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=5pt]},
   nd/.style={circle, draw=inkmuted, minimum size=7pt, inner sep=0pt},
   rt/.style={circle, draw=rust, fill=rustsoft, minimum size=9pt, inner sep=0pt}]
  % before
  \begin{scope}
   \node[font=\footnotesize\scshape] at (1.6,2.6) {a branch dies};
   \node[rt] (r) at (1.6,2.0) {};
   \node[nd] (a) at (0.6,1.1) {};   \node[nd] (b) at (2.6,1.1) {};
   \node[nd] (a1) at (0.1,0.2) {};  \node[nd] (a2) at (1.1,0.2) {};
   \draw[inkmuted] (r)--(a); \draw[inkmuted] (r)--(b);
   \draw[rust, densely dashed] (a)--(a1);  \draw[rust, densely dashed] (a)--(a2);
   \node[rust] at (0.6,1.1) {}; \draw[rust,line width=1pt] (0.35,0.85)--(0.85,1.35);
   \draw[rust,line width=1pt] (0.35,1.35)--(0.85,0.85);
   \node[rust, font=\scriptsize] at (0.6,1.55) {};
  \end{scope}
  % after
  \begin{scope}[shift={(5.2,0)}]
   \node[font=\footnotesize\scshape] at (1.6,2.6) {it re-subscribes};
   \node[rt] (R) at (1.6,2.0) {};
   \node[nd] (B) at (2.6,1.1) {};
   \node[nd] (A1) at (0.1,0.2) {};  \node[nd] (A2) at (1.1,0.2) {};
   \draw[inkmuted] (R)--(B);
   \draw[rust, ->, line width=0.9pt] (A1) to[bend left=10] (R);
   \draw[rust, ->, line width=0.9pt] (A2) to[bend right=5] (R);
   \node[inkmuted, align=center, font=\scriptsize] at (1.4,-0.5)
     {orphans reattach to a living relative;\\ a short cache fills the gap};
  \end{scope}
\end{tikzpicture}
\caption[Self-healing delivery]{No central repair coordinator. When a relay in a
topic's delivery tree disappears (left, the crossed node), the participants beneath
it simply re-issue their subscription, which lands on whichever relative is still
alive (right); a short cache of recent messages covers what was in flight during
the break. The mechanism that builds the tree is the mechanism that heals it.}
\label{fig:heal}
\end{figure}
...
This document draws its intellectual lineage from four sources, woven throughout
...
Guide, and the AI Grounding file. Axona is live and moving to full production; its
source is available at github.com/axona-net. The system described here is the
