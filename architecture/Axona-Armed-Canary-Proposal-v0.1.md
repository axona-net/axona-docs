# Arming hold-or-improve: constants and the two-relay canary (v0.1)
...
**Prepare** (code ready, inert until armed):
- `createRelay` gains env-driven arming options (`RELAY_SYNAPTOME_MAINTAIN`,
  ...
  off. Passing the options against the current 4.62.2 vendor is inert — an
  unknown constructor option is discarded — so the plumbing ships ahead of
  any vendor change without risk.
...
**Observe** — the soak's questions, each with a measurable answer:
- Storm indicators: connection-attempt rates from both canaries' logs, and
  the guard ledger (attempts, expiries, refills, coalesced) sampled
  periodically. Bounded and quiet = pass.
- The valve, live: each canary's watermark for the OTHER advances on its
  restarts; a mid-soak restart of canary 2 is the deliberate test. This is
  the first live exercise of the presence valve — the second prerequisite.
- Sustained evidence: 24–48 h of maintenance-under-real-churn without a
  drained table is the sustained case a 20-minute leaf could not show — the
  first prerequisite.
