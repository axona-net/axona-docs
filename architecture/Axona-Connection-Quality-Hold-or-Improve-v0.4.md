# Connection quality under hold-or-improve (v0.4)

**Status:** council-agreed definition draft, revision 4 · **Date:** 2026-08-23 ·
**Kernel on testnet:** 4.64.0 · **Policy set by:** David ·
**Supersedes:** v0.3 (axona-docs `e8d6c96`), v0.2 (`6e508e4`), v0.1 (`1811fc9`),
all left in place as record ·
**Revision driver (v0.4):** Aster `d8f26a6e`, Vega `c8e386c4` concurring — the
v0.3 reset record rate-limited relays but not direct receipt, so an origin
restarting in a loop could refill its neighbours' dial budgets once per
restart. Fixed here with a receiver-side, per-origin pacing bound, and in the
matrix at kernel test commit `e4b0edc` (scenario 2c). Earlier drivers: the
two `ff709b85` remainders (closed at v0.3 / `91fbca4`, Aster `d8f26a6e`, Orion
`a43d5c4e`, Vega `c8e386c4`); ten closure findings, Vega `76a300b4`; reset
requirement, Aster `ac99fe7f`, Orion `53e6daa4`, Vega `2ea89fc9`.

**What changed from v0.3:** inside "The reset record" only — receiver
verification gains a pacing step, the storm-restart bounds go from four to
five, the matrix paragraph gains the flood arm and states the guard model's
scope, and the verification table gains the flood row. Nothing else moved.
...
the watermark to gen 30 with a replay of 30 resetting nothing.
