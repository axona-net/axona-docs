# The pub/sub workload harness — v0.3 (Aster's consolidated response folded)

*2026-08-27 · kernel 4.68.2 on testnet · supersedes v0.2 (7a600e5) · folds
Aster's consolidated response (David terminal relay — she is not
re-attempting mesh publication; the relay counts as her seat under the
standing ruling) on top of e33fc610 / 730ff2b6 / 922dcd16 · her final
disposition: the sidecar-plus-telemetry design IS capable of covering the
failure surface once the criteria are frozen and the detector self-tests
pass; no objection to testnet arming under David's scope*
...
- **Churn collapse** — recovery time and backlog completeness after scripted
  joins, leaves, reconnects, relay restarts, and host loss.
- **Hours-long propagation** — late observations are RETAINED and
  reclassified rather than the classification ending at the SLO boundary; a
  message that arrives four hours late is a distinct finding from one that
  never arrives, and the harness must know the difference.
...
**Denominator contract (Aster §5) — defined BEFORE "100% delivery" means
anything.** The reconciliation window and a required-reader AVAILABILITY
contract are declared per topic up front. A subscriber deliberately down
during a kill/restart is not a live-latency sample until it returns; its
missing backlog is an eventual-replay integrity sample. Four figures are
reported separately, never merged: live delivery to readers available at
publish time; full-set completion; eventual replay after unavailable readers
return; operations remaining indeterminate. Zero operations may remain
indeterminate at disposition time.

| Gate | Threshold |
|---|---|
| Delivery | 100.0% of confirmed publishes reach the FULL predeclared required-reader set per topic under the denominator contract above; scored on full-set completion, never "any subscriber" |
...
**Window.** 12h minimum per arm — a floor, not a completion condition; Arm A
extends until the representativeness gate is met (both failure categories
AND the #51-shaped role-count behavior exercised) or the redesign verdict is
reached. Arm B covers the same scripted churn schedule and event counts.

**Provisional thresholds (Aster §6).** The p99 ≤ 2500ms and zero->10s tails
figures are PROVISIONAL until a short calibration run verifies clock error
and workload cadence. Thresholds may change only BEFORE Arm A — never after
comparative results are visible.

**Detector self-tests (Aster §7) — precondition for ANY arm.** Before Arm A,
one synthetic instance of EVERY failure class is injected into the analyzer
and logging pipeline, and each must produce its expected classification —
this is what separates "the network did not fail" from "the harness could
not notice the failure." Raw artifacts and the analyzer version/hash are
preserved; a SECOND analyzer pass over the same immutable JSONL manifest
must reproduce the disposition.

**Six separate dispositions (Aster §10).** (1) definition readiness,
(2) build clearance, (3) Arm A start, (4) Arm B testnet arming,
(5) evidence disposition, (6) production promotion. Definition concurrence
is not build clearance; build clearance is not authorization to arm; testnet
arming is not production clearance; kernel release clearance is not
deployment approval. Each is its own gate, (3), (4) and (6) David's.
