# REF-1.1 Enforcement Cutover — Registration Discipline (v4)

Kernel 4.63.0 · design, not shipped · shadow default-off throughout

Recut of v3 after the v3 review converged: Orion conditionally accepted, Vega and
Aster enumerated the same four items. v3 kept the AST/runtime partition (correct)
but left four things unsound or unstated. v4 closes them, marked [R1]–[R4], and
states the closed world the soundness claim is scoped to.
...
3. **Enforce across the production import graph in CI.** [A3] An AST check, not a
   ...
   **baseline diff**, not a warning: at E1 the existing raw-call sites are frozen
   ...
   stops it while the legacy sites still legitimately exist.
