# Connection quality under hold-or-improve (v0.3)

**Status:** council-agreed definition draft, revision 3 · **Date:** 2026-08-23 ·
**Kernel on testnet:** 4.64.0 · **Policy set by:** David ·
**Supersedes:** v0.2 (axona-docs `6e508e4`) and v0.1 (`1811fc9`), both left in
place as record ·
**Revision drivers (v0.3):** the two remainders from Vega `ff709b85`, concurred
Aster `770adef8`/`35027d4c`, Orion `00c0e96b` — (1) the matrix must model the
lane reserve-from-cap with qualification, now done at kernel test commit
`91fbca4`; (2) the candidate-reset record must be a named protocol object, done
here in "The reset record". Earlier drivers: ten closure findings, Vega
`76a300b4`, concurred Aster `11a43ca8`, Orion `2bc561e7`; reset requirement,
Aster `ac99fe7f`, Orion `53e6daa4`, Vega `2ea89fc9`.

**What changed from v0.2:** one new section, "The reset record", which names
the frame, its fields, who signs it, how it is verified, how far it travels,
and why it cannot restart the storm. The candidate-reset paragraph under the
liveness clauses now points there. The verification table gains the Sybil arm
and the reserve-from-cap assertion. Nothing else moved.
...
On success the receiver sets the watermark to `gen` and clears the
candidate's attempt-guard entry — one fresh budget of `maxAttempts`, nothing
more. If the receiver holds no guard entry for that nodeId the record updates
the watermark and does nothing else. A presence record is **not** a
...
fail verification at the next hop.

**Why it cannot restart the storm.** Four bounds, each local:
...
- An origin refreshing itself refills at most one `maxAttempts` budget per
  receiver per `gen`, and its dials into that budget still run under the
  per-node in-flight cap.
- An origin that keeps failing to bind after a reset is back to exhausted
  after `maxAttempts`, and the next reset costs it another `gen` — the
  per-origin rate limit at every relay bounds how often that can happen.
...
5 rejected while still expired, 6 accepted). The matrix models the record as
the guard's `onFreshRecord(nodeId, gen)`; the signature and binding checks
are the handshake's and are covered by the transport auth smokes.
