# Connection quality under hold-or-improve (v0.8)

**Status:** council-agreed definition draft, revision 8 · **Date:** 2026-08-24 ·
**Kernel on testnet:** 4.67.0 · **Policy set by:** David ·
**Supersedes:** v0.7 (axona-docs `66f50bc`), v0.6 (`0e4d75a`), v0.5
(`e96b67d`), v0.4 (`12dbe13`), v0.3 (`e8d6c96`), v0.2 (`6e508e4`), v0.1
(`1811fc9`), all left in place as record ·
**Revision driver (v0.8):** the slice-2 review's ratification condition —
Aster `6fa5abee` ("the `hop` sibling remains ratified only on the stated
condition that the next specification amendment explicitly records it
outside the signed transcript"), Vega `8e6f35e9`, Orion `3f5349f2`
concurring. The relay-depth marker is now named in "How far it travels."
Earlier drivers: v0.7 margin boundaries (Aster `b41e2a88`, Vega `9c423a4e`,
Orion `3b2cf359`); v0.6 one-key coherence (Aster `e4a753a5`); v0.5 scope
split (Aster `7e3836a9`, Vega `333688fe`); v0.4 direct-receipt pacing
(Aster `d8f26a6e`); the two `ff709b85` remainders; ten closure findings,
Vega `76a300b4`.

**What changed from v0.7:** one paragraph in "How far it travels" — the
`hop` sibling field is recorded explicitly: where it lives (outside the
signed transcript), what it carries, and why a lie about it grants no new
power. Nothing else moved.
...
over-represented band, and the candidate is admitted only when the victim's
band count exceeds the candidate's by the **anti-oscillation integer margin**
— at least 2. (v0.7; the v0.6 sentence said "fills a sparser band than the
victim occupies," which is +1, and a +1 pair oscillates: the evicted edge is
immediately re-admissible against its replacement.) The boundaries, each
pinned by a kernel test:

- **V = C+1 — refused.** Admitting would let the evicted edge reverse the
  swap on its next bind: structural ping-pong.
- **V = C+2 — admitted.** The post-swap bands tie at C+1. The tie is the
  point: the evicted edge is NOT re-admissible against the candidate, so the
  swap is final and the densest band shrank.
- **V = C+3 or more — admitted.** The candidate's band stays strictly
  sparser after the swap.

A candidate that ties the structure under the margin is refused at
admission; measured-performance comparison ("ties the structure and clears
the within-band margin") ranks incumbents only — a candidate has no history
to clear it with.
...
**The `hop` sibling.** (v0.8; ratified Aster `6fa5abee`, Vega `8e6f35e9`,
Orion `3f5349f2`.) The wire payload carries one field the transcript does
not: `hop` — 0 on the origin's own send, 1 on a relayed copy. Only hop-0
records are relayed, so honest depth is one. The field sits OUTSIDE the
signed transcript, the same placement as the hello's `pow` field, because a
relay cannot mutate a signed transcript without invalidating the origin's
signature — "forward the record unchanged" is a statement about the SIGNED
record, and `hop` is how a receiver tells an origin send from a relayed copy
without touching it. A receiver that lies — resetting hop to 0 before
forwarding — has re-originated a valid record, a power every neighbour
already has by holding the record at all; the per-origin relay rate limit
and the receiver's pacing step bound that exactly as they bound the honest
case.
...
| margin boundary trio (kernel gate smoke, `f92c13a`) | V=C+1 refused; V=C+2 admitted with post-swap tie and the evicted edge not re-admissible; legacy hex-keyed entries decided over, never thrown on | V=C+3 admitted with the candidate's band strictly sparser |
