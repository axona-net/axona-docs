# Arming hold-or-improve: constants and the two-relay canary (v0.3)
...
fact; (3) the automated proof exists: `test/fence_arming_gate.mjs`, 18
checks, covering the env→constants mapping verbatim, the version gate, and
the exact 4.62.2 silent-discard shape her review named; (4) the pass,
sampling, retention, and abort criteria are NUMERIC — "bounded and quiet"
is retired.
...
- (v0.3) Launch is self-proving: `createRelay` asserts post-construction and
  ...
  `test/fence_arming_gate.mjs` (18 checks), run before launch and recorded
  in the RUN-LOG.
...
**The numbers** (v0.3 — the auditable form of the criteria above; every
threshold falsifiable from retained artifacts):
...
| Guard over-budget | per-candidate attempts from guard state, sampled with the ledger | max attempts per candidate ≤ 4 for the entire soak | any candidate > 4 (guard defect — abort and investigate) |
