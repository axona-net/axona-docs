# Connection quality under hold-or-improve (v0.5)

**Status:** council-agreed definition draft, revision 5 · **Date:** 2026-08-23 ·
**Kernel on testnet:** 4.64.0 · **Policy set by:** David ·
**Supersedes:** v0.4 (axona-docs `12dbe13`), v0.3 (`e8d6c96`), v0.2 (`6e508e4`),
v0.1 (`1811fc9`), all left in place as record ·
**Revision driver (v0.5):** Aster `7e3836a9` (correcting `ae507c77`), Vega
`333688fe` — the same-nodeId reset storm closed at v0.4 (Aster, Vega, Orion
`341b0ca2`); what remained was to say the scope split out loud: a presence
reset for a nodeId the receiver gave up on is one mechanism with one bound; a
fresh identity joining is another mechanism with another bound, and the two
must not be confused. Said here, with the keying refinement that falls out of
how nodeIds are built, and tested at kernel test commit `5ef81ef` (scenario
2d). Earlier drivers: v0.4 direct-receipt pacing (Aster `d8f26a6e`, Vega
`c8e386c4`); the two `ff709b85` remainders (closed at v0.3 / `91fbca4`); ten
closure findings, Vega `76a300b4`; reset requirement, Aster `ac99fe7f`.

**What changed from v0.4:** inside "The reset record" only — two new
paragraphs ("Scope: two paths, two bounds" and "What the key is"), the matrix
paragraph gains scenario 2d, and the verification table gains its row. Nothing
else moved. The council's choice on record: keep identity keying, do NOT
re-key to anything more persistent than the node's own key — that would be
the cross-session correlator the protocol refuses.
...
its degree floor. It is never persisted. Node and transport ids are ephemeral
per session by rule — a stable id is a correlator — so a new session is a new
nodeId with a fresh watermark on every receiver, and cross-session
monotonicity is neither needed nor wanted. `nonce` makes each record
byte-unique for dedup; `gen` is the freshness proof. Two records with the
same gen are the same record, whatever their nonces.
...
3. `gen` is strictly greater than the receiver's watermark for this nodeId.
...
4. (v0.4.) **Pacing.** The receiver refills an origin's attempt budget at
   most once per origin per window (`T_refill`, a matrix parameter). A valid
   higher `gen` inside the window still advances the watermark — freshness is
   recorded — but refills nothing; it is coalesced into the refill the window
   already granted.

On success the receiver sets the watermark to `gen` and, if the window
allows, clears the candidate's attempt-guard entry — one fresh budget of
`maxAttempts`, nothing more. If the receiver holds no guard entry for that
nodeId the record updates the watermark and does nothing else.
...
at most one hop, to its own neighbours, rate-limited per origin nodeId per
...
The guard model takes `(nodeId, gen)` and nothing else. The signature and
pubkey-to-nodeId binding checks are the handshake's; they are covered by the
transport auth smokes and are not exercised by this matrix. A reviewer who
