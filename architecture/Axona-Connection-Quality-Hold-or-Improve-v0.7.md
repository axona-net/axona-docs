# Connection quality under hold-or-improve (v0.7)

**Status:** council-agreed definition draft, revision 7 · **Date:** 2026-08-24 ·
**Kernel on testnet:** 4.65.1 · **Policy set by:** David ·
**Supersedes:** v0.6 (axona-docs `0e4d75a`), v0.5 (`e96b67d`), v0.4
(`12dbe13`), v0.3 (`e8d6c96`), v0.2 (`6e508e4`), v0.1 (`1811fc9`), all left
in place as record ·
**Revision driver (v0.7):** implementation review of the first kernel slice
— Aster `b41e2a88`, Vega `9c423a4e`/`70f85cc7`, Orion `3b2cf359`. The
implemented swap bound is an integer margin the v0.6 sentence did not state,
and its boundary at exactly +2 produces a post-swap TIE, not "sparser." The
amendment names the margin and its three boundaries; the kernel boundary
tests pin them (testnet `f92c13a`). Earlier drivers: v0.6 one-key coherence
(Aster `e4a753a5`); v0.5 scope split (Aster `7e3836a9`, Vega `333688fe`);
v0.4 direct-receipt pacing (Aster `d8f26a6e`); the two `ff709b85`
remainders; ten closure findings, Vega `76a300b4`.

**What changed from v0.6:** the swap rule in "Admission at cap" now states
the anti-oscillation integer margin with its boundary semantics, replacing
"fills a sparser band than the victim occupies"; the verification table
gains the boundary-trio row. Nothing else moved.
