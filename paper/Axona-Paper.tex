...
$25\,000$ nodes \axona routes global lookups at $1.33\times$ the
Dabek $3\delta$ analytical floor for any recursive $O(\log N)$ DHT
--- within $68$\,ms of the theoretical lower bound --- versus
Kademlia's $4.13\times$.  An ablation that strips the geographic
prefix shows learning alone beats Kademlia by $39\%$.  Built-in
...
routes at $1.33\times$ the floor, within $68$\,ms of the
theoretical lower bound; Kademlia sits at $4.13\times$
...
geographic structure, \axona still routes $39\%$ faster than
...
\;\Vert\; \mathrm{hash}_{56}(\mathrm{topic\_name})
\]
...
\;\Vert\;
\underbrace{H(\mathrm{publicKey})}_{\text{56 bits}}
\label{eq:gdht-id}
\end{equation}
...
The remaining $56$ bits are the standard cryptographic hash of
the node's public key.  There is no other change to Kademlia ---
...
removed (geoBits$=0$), \axona still routes $39\%$ faster than
...
sampled per sweep over $10\,000$ random node pairs.  $25\,000$
node measurements are from sim v0.93.0 / \texttt{@axona/protocol}
v1.1.2 (May 2026).}
\label{tab:floor}
\small
...
\midrule
$5\,000$  & $67.9$ & $204$ & $410$\ ($2.01\times$) & $268$\ ($1.32\times$) & $\mathbf{240}$\ ($\mathbf{1.18\times}$) \\
$25\,000$ & $68.0$ & $204$ & $842$\ ($4.13\times$) & $826$\ ($4.05\times$) & $\mathbf{272}$\ ($\mathbf{1.33\times}$) \\
$50\,000$ & $68.9$ & $207$ & $548$\ ($2.65\times$) & $291$\ ($1.41\times$) & $\mathbf{264}$\ ($\mathbf{1.28\times}$) \\
\bottomrule
\end{tabular}
\end{table*}
...
\paragraph{\axona plateaus at the floor.}
At $25\,000$ and $50\,000$ nodes \axona sits at
$1.28$--$1.33\times$ the $3\delta$ floor.  Within the
geometric-series decomposition of Eq.~\ref{eq:dabek}, the
residual $\sim\!30\%$ overhead has a clean structural
explanation: \axona averages $4.5$--$5.5$ hops where a
Dabek-ideal protocol would take $\sim\!3$, and each ``extra''
hop costs $\sim\!\delta/2 \approx 34$\,ms --- exactly the
geometric tail the series predicts.
...
\textbf{Improvement} \\
\midrule
\KDHT & $852$ & $842$ & --- (baseline) \\
\GDHT & $1060$\textsuperscript{$\dagger$} & $826$ & --- (geo is mechanism) \\
\textbf{\axona} & $\mathbf{513}$ & $\mathbf{272}$ &
$\mathbf{-40\%}$ from learning \\
\bottomrule
...
\axona still routes $\sim\!40\%$ faster than Kademlia at
$25\,000$ nodes purely from learning.  Add the S2 prefix back
in and the gap widens to $68\%$.  This is the cleanest possible
``learning helps'' claim: every confound (bootstrap strategy,
identifier structure, geographic prefix) is controlled, and the
routing/learning algorithm still wins.  Geography helps;
geography is not necessary.

The opposite-sign result for \GDHT --- it gets \emph{worse} at
geoBits$=0$, from $826$ to $1060$\,ms --- is the consistency
check that the ablation is set up correctly: \GDHT's entire
strategy is ``follow the prefix.''  With no prefix to follow it
loses the locality benefit that defines it.
...
\KDHT & $4.12$ & $762$ \\
\GDHT & $5.10$ & $767$ \\
\textbf{\axona} & $\mathbf{4.38}$ & $\mathbf{239}$ \\
\bottomrule
\end{tabular}
\end{table}

The hop counts are close ($4.1$ to $5.1$), but \axona's
\emph{latency} is $3.2\times$ lower than either reference.  The
difference comes from where the routing decisions land: \axona's
AP scoring weights short-RTT edges so heavily that even a hop
``inefficient'' by raw count is short in wall-clock time.
...
within $1.28$--$1.33\times$ the Dabek $3\delta$ analytical floor
for recursive $O(\log N)$ routing at $50\,000$ nodes.
...
prefix shows learning alone outperforms Kademlia by $40\%$,
