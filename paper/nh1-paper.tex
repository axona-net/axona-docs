...
\paragraph{Bandwidth saturation.} AP scoring's static
$\frac{1}{2}^{(\ell/100)}$ latency penalty does not penalize overloaded
nodes. A real system risks \emph{success disasters}: LTP reinforces
a node until it congests, abandons it when it slows, then flocks back
when it recovers, in oscillatory loops. Mitigation: load-aware AP
scoring with effective delay $= \mathrm{base\_delay} \times (1 +
\mathrm{msg\_rate} / \mathrm{bandwidth\_cap})$ along the lines of
Makris~\emph{et~al.}'s DFE pattern~\cite{makris2017novel}.
